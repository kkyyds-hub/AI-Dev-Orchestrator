    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <h3 className={`text-lg font-semibold ${isLine ? "text-zinc-100" : "text-slate-50"}`}>
            {detail.title}
          </h3>
          <p className={`text-xs ${isLine ? "text-zinc-600" : "text-slate-500"}`}>
            Task ID：{detail.id}
          </p>
        </div>
      ...
      <div className="mt-4">
        <div className={`text-xs uppercase tracking-[0.18em] ${isLine ? "text-zinc-600" : "text-slate-500"}`}>输入摘要</div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <div>
          <div className={`text-xs uppercase tracking-[0.18em] ${isLine ? "text-zinc-600" : "text-slate-500"}`}>验收标准</div>
          {detail.acceptance_criteria.length > 0 ? (
            <ul className={`mt-2 space-y-2 text-sm ${isLine ? "text-zinc-300" : "text-slate-300"}`}>
            ...
            <p className={`mt-2 text-sm ${isLine ? "text-zinc-600" : "text-slate-500"}`}>暂无显式验收标准</p>
          )}
        </div>

        <div className="space-y-4">
          <div>
            <div className={`text-xs uppercase tracking-[0.18em] ${isLine ? "text-zinc-600" : "text-slate-500"}`}>依赖任务</div>
            {detail.depends_on_task_ids.length > 0 ? (
                  ...
                  <code
                    ...
                    className={
                      isLine
                        ...
                        : "block break-all rounded-lg border border-slate-800 bg-slate-900/70 px-3 py-2 text-xs text-cyan-200"
                    }
                  >
                    {dependencyId}
                  </code>
              ...
              <p className={`mt-2 text-sm ${isLine ? "text-zinc-600" : "text-slate-500"}`}>无前置依赖</p>
            )}
          </div>

          <div>
            <div className={`text-xs uppercase tracking-[0.18em] ${isLine ? "text-zinc-600" : "text-slate-500"}`}>暂停说明</div>
            <p className={`mt-2 whitespace-pre-wrap text-sm leading-6 ${isLine ? "text-zinc-300" : "text-slate-300"}`}>
