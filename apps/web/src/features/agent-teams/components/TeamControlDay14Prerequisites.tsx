...
export function TeamControlDay14Prerequisites(props: {
  ...
  return (
    <section
      ...
      className="rounded-2xl border border-[#333333] bg-[#181818] p-4 text-xs text-zinc-400"
    >
      <h3 className="text-sm font-semibold text-zinc-100">运行前置条件</h3>
      <dl className="mt-3 space-y-3">
        <div>
          <dt className="text-zinc-500">已启用角色代码</dt>
          <dd className="mt-1 max-h-24 overflow-y-auto break-words pr-2 leading-5 text-zinc-300">
            {props.snapshot.day14_prerequisites.enabled_role_codes.join(", ") || "无"}
          </dd>
        </div>
        <div>
          <dt className="text-zinc-500">预算策略项</dt>
          <dd className="mt-1 max-h-24 overflow-y-auto break-words pr-2 leading-5 text-zinc-300">
            {props.snapshot.day14_prerequisites.budget_policy_keys.join(", ") || "无"}
          </dd>
        </div>
        <div>
          <dt className="text-zinc-500">策略生效范围</dt>
          <dd className="mt-1 max-h-24 overflow-y-auto break-words pr-2 leading-5 text-zinc-300">
