  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className={`text-base font-semibold ${isLine ? "text-zinc-100" : "text-zinc-100"}`}>日志事件</h3>
        ...
        {selectedRun ? <StatusBadge label="运行日志" tone="info" /> : null}
      </div>
      ...
      {!selectedRun ? (
        ...
        <div className="mt-4 space-y-3">
          <div
            ...
            className={isLine ? "border-y border-[#333333] py-4" : "border-l border-[#333333] px-4 py-3"}
          >
            <div className={`text-xs uppercase tracking-[0.18em] ${isLine ? "text-zinc-600" : "text-zinc-200"}`}>
              运行日志上下文
            </div>
            <p className={`mt-2 text-xs ${isLine ? "text-zinc-500" : "text-zinc-400"}`}>运行 {selectedRun.id}</p>
          ...
          {decisionTraceQuery.data ? (
            <div className={isLine ? "border-y border-[#333333] py-4" : "rounded-xl border border-[#333333] bg-transparent p-4"}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h4 className={`text-sm font-semibold ${isLine ? "text-zinc-100" : "text-zinc-100"}`}>决策回放</h4>
              ...
              <div className={isLine ? "mt-4 divide-y divide-[#333333] border-y border-[#333333]" : "mt-4 space-y-3"}>
                {decisionTraceQuery.data.trace_items.map((item, index) => (
                  ...
                  >
                    ...
                    <p className={`mt-2 text-sm leading-6 ${isLine ? "text-zinc-300" : "text-zinc-400"}`}>{item.summary}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <InfoRow label="运行 ID" value={selectedRun.id} surfaceVariant={surfaceVariant} />
          ...
          {logsQuery.data?.events.length ? (
            <div className={isLine ? "max-h-[28rem] divide-y divide-[#333333] overflow-y-auto border-y border-[#333333]" : "max-h-[28rem] space-y-3 overflow-y-auto pr-1"}>
              {logsQuery.data.events.map((event, index) => (
                <div
                  ...
                  className={isLine ? "py-4" : "rounded-xl border border-[#333333] bg-transparent p-4"}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className={`text-sm font-medium ${isLine ? "text-zinc-100" : "text-zinc-100"}`}>{event.event}</div>
                      <div className={`mt-1 text-xs ${isLine ? "text-zinc-600" : "text-zinc-500"}`}>
                  ...
                  <p className={`mt-3 text-sm leading-6 ${isLine ? "text-zinc-300" : "text-zinc-400"}`}>{event.message}</p>

                  {Object.keys(event.data).length ? (
                    <pre className={isLine ? "mt-3 overflow-x-auto border-l border-[#333333] bg-transparent p-3 text-xs leading-6 text-zinc-400" : "mt-3 overflow-x-auto rounded-xl border border-[#333333] bg-transparent/80 p-3 text-xs leading-6 text-zinc-400"}>
            ...
            <div className={isLine ? "border-y border-dashed border-[#333333] py-6 text-sm text-zinc-500" : "rounded-xl border border-dashed border-[#333333] bg-transparent p-4 text-sm text-zinc-400"}>
...
function InfoRow(props: {
  ...
  return (
    ...
    >
      <div data-slot="label" className={`text-xs uppercase tracking-[0.18em] ${isLine ? "text-zinc-600" : "text-zinc-500"}`}>
        {props.label}
      </div>
      <div data-slot="value" className={`mt-2 break-all text-sm font-medium ${isLine ? "text-zinc-100" : "text-zinc-100"}`}>
        {props.value}
      </div>
    </div>
  );
