...
- 「新建项目会话」是用户进入工作流的最高频动作，因此使用白底黑字主按钮。
...
- 新建项目会话：引导创建项目，并自动创建该项目下的第一个 AI 主管对话。
...
- 新建项目会话、搜索框、用户菜单不进入滚动区。
- 运行与治理、主页面、项目会话在中间区域滚动。
- 隐藏可见滚动条，但保留鼠标滚轮和触控板滚动能力。

## 项目会话创建入口规则

- Sidebar 顶部主按钮命名为“新建项目会话”，语义是创建新项目并自动创建该项目下的第一个 AI 主管对话。
- 已有项目标题右侧提供 hover 出现的小加号，用于在该项目下追加新的 AI 主管对话。
- 点击项目标题用于展开 / 收起项目；点击项目小加号只新增会话，不触发折叠。
- 新建会话不会自动生成计划、任务队列或审查结果，只进入“欢迎 / 我们来构建什么？”状态。
