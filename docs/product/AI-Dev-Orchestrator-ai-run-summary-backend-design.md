...
> 设计版本：V1.0
> 当前阶段：阶段 2C-B（真实 DeepSeek 运行时联调验收）
> 后续阶段：阶段 2C-C（运行时失败回退与体验收口）
...
| 阶段 2C-A-R2 | 测试补强与文档追溯修正 | 已完成 |
| 阶段 2C-B | 真实 DeepSeek 运行时联调验收 | 已完成 |
| 阶段 2C-C | 运行时失败回退与体验收口 | 未开始 |
...
> - provider_key 测试真正覆盖 _call_provider_text → generate_text 路径
> - env-only 测试真正覆盖 provider_config_service 路径
> - 未真实调用外部 AI，未进入 2C-B
>
...
>
> 结论：**Pass** — 真实 DeepSeek 成功生成 source=ai 运行摘要
>
> 运行时验收结果：
> - Provider 配置：DeepSeek，masked_key=********0780
> - Connectivity test：passed（auth_valid=true, api_family=chat_completions, latency=2401ms）
> - 使用 run_id：`fa552a28-9502-40c9-b05f-19585fe8ea5d`（succeeded, deepseek/deepseek-v4-pro）
> - GET /ai-summary：只读，初始返回 rule_fallback active_summary
> - POST /regenerate：200，source=ai，model_provider=deepseek，model_name=deepseek-v4-pro
> - provider_receipt_id：`ec7b2cb9-c2b5-4110-bbb8-0355d4f4e308`（非空）
> - prompt_hash 与 rule_fallback 不同，代表真实 AI prompt
> - Markdown 含完整五标题，中文，非 JSON，非代码块包裹
> - GET 再次读取：active_summary=ai，已持久化
> - 测试：35 passed，build：通过
> - 未暴露 API key
