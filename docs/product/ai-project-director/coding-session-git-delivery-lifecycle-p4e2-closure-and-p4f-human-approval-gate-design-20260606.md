...
> **状态**: P4-E2 Closure: Pass；P4-F Design R1: Design only；AI Project Director 总闭环 Partial
...
| 模型 | 责任 |
|------|------|
| `HumanApprovalRecord` | 表达一次用户确认记录，包含审计、scope、过期、撤销、是否已被后续阶段消费等生命周期字段 |
| `DeliveryHumanApprovalResult` | 表达 P4-F gate 评估结果，用于后续 Worker / API 只读透传；可嵌入或展开 `HumanApprovalRecord` 字段 |
| `HumanApprovalGateBuilder` | 从 P4-C/P4-D evidence + 用户确认记录输入中评估是否形成有效 human approval evidence |
| `HumanApprovalGateSafetyFlags` | 强制 P4-F 不执行 Git 写入，只允许形成“可进入下一阶段 guardrail”的证据 |
...
| `approval_id` | `str | None` | 确认记录 ID；新建时由后端生成，重放 / 查询时必须稳定 |
...
| `approval_actor_id` | `str | None` | 发起确认的用户 ID；输出中规范化为 `approved_by` |
...
| `approval_scope` | `str | None` | 确认范围；P4-F 仅允许 `git_add_commit_preview`，不能包含 push / PR / CI |
| `approval_expires_at` | `datetime | None` | 确认过期时间；必须晚于 `approval_created_at`，用于防止陈旧确认继续流转 |
| `approval_applied` | `bool` | 是否已被后续真实写入阶段消费；P4-F 新确认必须为 False |
| `approval_revoked` | `bool` | 确认是否已撤销；P4-F ready=True 时必须为 False |
...
9. 用户确认必须包含明确 actor、动作、请求 ID、确认时间、确认范围、过期时间。
10. `approval_scope` 必须与 `approval_requested_action`、`operation_dry_run.proposed_operation` 一致。
11. `approval_expires_at` 必须存在且晚于 `approval_created_at`。
12. `approval_applied` 必须为 `False`，避免重复消费旧确认。
13. `approval_revoked` 必须为 `False`。
14. 用户确认的文件列表和提交说明必须与 evidence 一致。
...
1. `HumanApprovalRecord`：不可缺少的用户确认审计记录与生命周期状态。
2. `DeliveryHumanApprovalResult`：gate 评估结果，引用或展开 `HumanApprovalRecord`，并在后续接入 Worker / API 时以 `delivery_human_approval_*` 或 `human_approval_gate_*` 前缀只读透传。

为避免字段漂移，后续实现必须保证 input / output 字段名称一致：输入中的 `approval_id`、`approved_by` / `approval_actor_id`、`approval_scope`、`approval_expires_at`、`approval_applied`、`approval_revoked` 必须在输出中可审计。
...
| `source` | `str` | 固定为 `delivery_human_approval` 或 `human_approval_gate`，实现时二选一并保持 API 前缀一致 |
...
### 5.2 HumanApprovalRecord 字段

| 字段 | 类型 | 说明 |
|------|------|------|
| `approval_id` | `str` | 确认记录唯一 ID；用于审计、幂等、撤销、后续消费追踪 |
| `approved_by` | `str` | 实际确认用户 ID；由输入 `approval_actor_id` 规范化而来 |
| `approved_by_display_name` | `str | None` | 确认用户展示名 |
| `approval_scope` | `str` | 确认范围；P4-F 仅允许 `git_add_commit_preview` |
| `approval_requested_action` | `str` | 被确认动作；P4-F 仅允许 `approve_git_add_commit_preview` |
| `approval_client_request_id` | `str` | 前端确认请求 ID，用于幂等 |
| `approval_created_at` | `datetime` | 确认创建时间 |
| `approval_expires_at` | `datetime` | 确认过期时间；过期后不得进入下一阶段 guardrail |
| `approval_applied` | `bool` | 是否已被后续真实写入阶段消费；P4-F 输出 ready=True 时必须为 False |
| `approval_revoked` | `bool` | 是否已撤销；P4-F 输出 ready=True 时必须为 False |
| `approval_confirmation_fingerprint` | `str` | 对确认文本、文件列表、提交说明、scope、expires_at 形成的摘要，不保存敏感长文本 |

### 5.3 DeliveryHumanApprovalResult 确认字段

| 字段 | 类型 | 说明 |
|------|------|------|
| `approval_required` | `bool` | 是否需要用户确认；P4-F ready 前后均应为 True |
| `approval_granted` | `bool` | 用户是否已明确确认；P4-F ready=True 时为 True，但不表示已写仓库 |
| `approval_id` | `str | None` | 对应 `HumanApprovalRecord.approval_id` |
| `approved_by` | `str | None` | 对应 `HumanApprovalRecord.approved_by` |
| `approved_by_display_name` | `str | None` | 对应确认用户展示名 |
| `approval_scope` | `str | None` | 对应确认范围 |
...
| `approval_expires_at` | `datetime | None` | 确认过期时间 |
| `approval_applied` | `bool` | 是否已被后续阶段消费；P4-F ready=True 时必须为 False |
| `approval_revoked` | `bool` | 是否已撤销；P4-F ready=True 时必须为 False |
| `approval_confirmation_fingerprint` | `str | None` | 确认摘要指纹 |
...
| `approval_applied` | False | 用户确认尚未被后续真实写入阶段消费 |
| `approval_revoked` | False | 用户确认未撤销；撤销后不得 ready |
| `approval_expired` | False | 用户确认未过期；过期后不得 ready |
...
- `approval_applied=True`、`approval_revoked=True` 或 `approval_expired=True` 时必须 `ready=False`。
- `gate_allows_next_guardrail=True` 仅允许在 `ready=True`、`approval_applied=False`、`approval_revoked=False`、`approval_expired=False` 时出现。
...
| `approval_id_missing` | 缺少确认记录编号 | 查询 / 复用既有确认记录时 `approval_id` 缺失，或新建后无法生成 ID |
| `approval_actor_missing` | 缺少确认用户 | `approval_actor_id` / `approved_by` 缺失 |
| `approval_scope_missing` | 缺少确认范围 | `approval_scope` 缺失 |
| `approval_scope_unsupported` | 确认范围不受支持 | `approval_scope` 不是 `git_add_commit_preview` |
| `approval_scope_mismatch` | 确认范围与预览动作不一致 | `approval_scope`、`approval_requested_action`、`proposed_operation` 无法互相匹配 |
...
| `approval_expiry_missing` | 缺少确认过期时间 | `approval_expires_at` 缺失 |
| `approval_expired` | 用户确认已过期 | 当前时间已晚于 `approval_expires_at` |
| `approval_already_applied` | 用户确认已被使用 | `approval_applied is True` |
| `approval_revoked` | 用户确认已撤销 | `approval_revoked is True` |
...
7. 展示确认审计字段时使用安全中文标签：确认编号、确认人、确认范围、确认时间、过期时间、是否已使用、是否已撤销。
8. `approval_applied=false` 建议展示为“尚未用于真实写入”；`approval_revoked=false` 建议展示为“未撤销”；`approval_expires_at` 必须展示为“确认有效期”。
9. 不得把 `approved_by` 展示成“提交人”或“代码作者”，只能展示成“确认人”。
...
| 成功反馈 | 表达为“已记录用户确认，可进入下一阶段检查”，并展示确认编号、确认范围、确认有效期 |
...
- `content_detail` 只包含 evidence 字段、approval_id、approved_by、approval_scope、approval_expires_at、approval_applied、approval_revoked、request_id、fingerprint、reason_code
- 不写入完整敏感确认文本
- 不写入任何声称 Git 已写入的内容
...
- approval_id missing / generated。
- approved_by / actor missing。
- approval_scope missing / unsupported / mismatch。
- approval_expires_at missing / expired。
- approval_applied=True blocked。
- approval_revoked=True blocked。
- confirmation missing。
- changed files mismatch。
- commit message mismatch。
- forbidden safety flags 强制校验。
- ready=True 时 `approval_id`、`approved_by`、`approval_scope`、`approval_expires_at` 完整输出。
- ready=True 时 `approval_applied=False`、`approval_revoked=False`。
...
- TaskWorker 成功路径中基于 P4-C/P4-D evidence 与用户确认记录生成 `delivery_human_approval_*` 或 `human_approval_gate_*` 字段，且包含 `approval_id`、`approved_by`、`approval_scope`、`approval_expires_at`、`approval_applied`、`approval_revoked`。
...
- 成功后展示 approval evidence，包括确认编号、确认人、确认范围、确认有效期、是否已使用、是否已撤销。
...
|------|------|
| P4-E2 Closure | Pass |
| P4-F Human Approval Gate Design R1 | Pass |
| 是否改代码 | 否 |
| 是否改前端 | 否 |
| 是否运行全量测试 | 否 |
| 产品运行时是否实现 Git 写操作 | 否 |
