...
| `tests/test_project_director_task_creation.py` | 18 tests (13 Phase1 + 5 hardening, including 1 event-consistency test) |
...
============================= 94 passed (historical global regression; not rerun in this patch) =============================
```

### Test coverage (Phase1 13 + Hardening 5)
...
| `test_create_tasks_atomic_task_count_matches_record` | 原子性：Task 数量与 TaskCreationRecord 一致 |
| `test_empty_proposed_task_description_falls_back_to_title` | 空 description → input_summary 兜底 |
| `test_duplicate_create_tasks_still_returns_409` | 重复创建仍 409（hardening 后） |
| `test_atomic_rollback_on_record_creation_failure` | Record creation failure leaves no Task rows and emits no ghost task_created events |
| `test_create_tasks_publishes_events_only_after_commit` | Success path emits created events only after TaskCreationRecord commit; event IDs match DB Task rows |

---

## 9. BCG-04A Hardening Patch（2026-05-20）

### 性质

后端闭环加固（不新增功能，不新增 API，不改前端）。

### 补强内容

1. **原子事务**：Task 和 TaskCreationRecord 使用同一个 SQLAlchemy session 写入，Task 通过 `add_no_commit` 添加，TaskCreationRecord 的 `create()` 作为唯一 commit 点。任何一步失败，session 回滚，不残留部分 Task。

2. **前置预校验**：`_validate_proposed_tasks()` 在任何 DB 写入之前校验所有 proposed_tasks 的 title 非空、role_code 合法、priority_hint 可映射。校验失败直接返回错误，不写入任何数据。

3. **空 description 兜底**：`_build_input_summary()` 当 ProposedTask.description 为空时，自动使用 `"由计划版本生成的任务：{title}"` 作为 Task.input_summary，避免中途抛错。

4. **TaskRepository 扩展**：新增 `add_no_commit()` 方法，flush 但不 commit。保留原有 `create()` 语义不变。

5. **Event consistency patch (2026-05-24)**: `add_no_commit()` no longer publishes task-created events. BCG-04A publishes via `publish_created()` only after TaskCreationRecord commit succeeds. If record creation fails and the session rolls back, there are no Task rows and no ghost SSE/console events.

### 测试结果

- Task creation tests: 18/18 passed (5 hardening tests, including event-consistency).
- Global regression: not rerun in this patch; historical record was 94/94 passed (38 + 24 + 15 + 17).
- 原有 13 个测试全部继续通过
...
- 原子事务：Task + TaskCreationRecord 同 session 单 commit
- 前置预校验：非法 proposed_task 在 DB 写入前被拦截
- 空 description 兜底：不抛错，使用 title 作为 input_summary
- Event consistency: created events are emitted only after commit; rollback leaves zero `TaskTable` rows and zero ghost events.
- 状态约束（404/409）覆盖全面
- 重复创建防护（409 idempotency guard）
- project_id 缺失防护（409）
- source_draft_id + TaskCreationRecord 双追溯
- 17 个测试全部通过，原有 77 个测试无回归
