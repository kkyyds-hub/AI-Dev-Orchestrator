...
| BCG-12 | 文件定位与上下文包 | 仓库工作区 | file-locator、context-pack | 后端存在 | Backend Pass / Runtime Evidence Partial (Security Gap) | 2026-05-24 BCG-12A live evidence: file locator (3 query types) + context pack (5 files, 4 languages) + path safety Pass; Security Gap: node_modules/__pycache__ files readable via context-pack API; budget truncation verified by test suite (sample files too small for live trigger) | AI 修改文件前缺可靠上下文（定位已通过）；ignored directory 文件可被读取 | P0 证据 | 补 context-pack ignored directory 过滤 + 接 BCG-13 |
...
| BCG-12 | 文件定位与上下文包 | BCG-12A live evidence已完成 (157/157, Security Gap: ignored dir files readable) |
| BCG-13 | 变更计划与变更批次 | 待做 |
...
## 12C. BCG-12A Runtime Evidence — File Locator + Context Pack (2026-05-24)

Scope: DeepSeek live evidence script validating the full file-locator → context-pack
chain against the BCG-11A bound sample repository.
...
- **Snapshot verification**: Pass (status=success, scan_error=null, file_count=5, tree includes README.md/src/web/config/docs, language_breakdown covers Markdown/Python/TypeScript/JSON, ignored dirs exclude .git/node_modules/__pycache__).
...
- **Budget truncation**: Partial (sample files are ~215 bytes, below API minimum 512; truncation logic verified by `test_repository_context_pack_api.py`).
- **Path safety (../ traversal)**: Pass (422).
- **Path safety (absolute path)**: Pass (422).
- **Ignored directory file access (node_modules)**: **Security Gap** — `node_modules/ignored.js` readable via context-pack API (200, included_file_count=1).
- **Ignored directory file access (__pycache__)**: **Security Gap** — `__pycache__/ignored.py` readable via context-pack API (200, included_file_count=1).

### Gate

**BCG-12 Runtime Evidence: Partial (File locator + context pack evidence Pass / Security Gap: ignored directory files readable via context-pack)**

Context-pack service validates path traversal (../, absolute) but does not cross-reference
ignored_directory_names. Files in node_modules/__pycache__ are readable when explicitly
selected. This is a defense-in-depth gap, not a path escape.
