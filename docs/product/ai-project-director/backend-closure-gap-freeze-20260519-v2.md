...
| BCG-14 | 预检与人工放行 | 仓库工作区 / 审批 | preflight、repository-preflight actions | BCG-14A-R1 closeout: all gaps closed. ready_for_execution (small scope, 0 findings), manual reject (manual_rejected, decision persisted), NOT_STARTED action 422, inbox/day15-flow read-back. 59/59 passed, 0 gaps. | Backend Pass / Runtime Evidence Pass | R1 closeout 完成；ready/blocked/approve/reject 四态 + illegal-action protection + read-back 全部验证 | P1 证据 / P0 收口 | BCG-14 Runtime Evidence Pass；接 BCG-15 提交草案 |
...
| BCG-16 | 本地 Git 写入 apply-local / git-commit | 仓库工作区 | `POST /repositories/change-batches/{id}/apply-local`、`git-commit` | BCG-16A live evidence: isolated repo + full guard chain (preflight ready → verification passed → commit candidate → release gate approved) → apply-local (applied, verification_passed, files written) → git-commit (committed, commit_sha, staged clean, no remote). Path safety (../, .git, absolute) all blocked. 55/55 passed, 0 gaps. | Backend Pass / Runtime Evidence Pass / Frontend Entry Pending | BCG-16A 验证了完整 apply-local + git-commit 真实链路；路径安全、guard chain、no-push 边界全部通过 | P0 证据 / P0 收口 | BCG-16 Runtime Evidence Pass；后续 BCG-17 仍 Deferred（remote push / PR / merge 不在本阶段） |
