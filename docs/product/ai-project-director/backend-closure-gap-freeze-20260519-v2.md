...
|---|---|---|---|---|---|---|---|---|---|
| BCG-01 | AI 项目主管目标澄清与会话闭环 | `/workbench` | 暂未看到明确 Project Director 顶层会话 API | Agent thread 存在，但偏运行会话/干预 | Partial | 缺“用户目标 → AI 澄清 → 待确认事项 → 用户确认 → 计划生成”的一体化后端模型 | 用户仍觉得是自己拆任务，不是 AI 主管主动推进 | P0 | 新增 Project Director session Phase1，只做目标澄清/确认，不改任务/仓库 |
| BCG-02 | 计划版本与确认审计 | 项目创建 / 计划草案入口 | `POST /planning/drafts`、`POST /planning/apply` | Planner draft/apply 存在，会创建项目/任务 | Partial | apply 会落任务，但缺显式 PlanVersion / PlanApproval / user_confirmed_at 审计层 | 计划与任务生成之间缺可追溯确认点 | P0 | 补计划确认记录或把 apply 结果沉淀为 plan version 证据 |
| BCG-03 | 待确认事项 / 人工决策池 | 工作台 / 执行中心 | task human status、agent interventions 部分存在 | 分散在 task/agent/preflight/approval 中 | Partial | 缺统一 pending confirmations inbox，把计划确认、预检确认、审批返工、人类干预统一收口 | 用户不知道哪些事项等自己决定 | P0 | 新增只读 confirmation inbox 聚合接口；先聚合现有数据，不新增写动作 |
...
| BCG-05 | Worker 调度与运行生成 | 工作台 / 执行中心 | `POST /workers/run-once`、`POST /workers/run-pool-once` | 单次/池化调度存在，返回 run/strategy/role/skill/memory/cost/log | Backend Pass / Runtime Evidence Missing | 需要当前环境真实 provider_reported run 证据 | 页面有运行，但总 Gate 没证据 | P0 证据 | 配置 provider，准备任务，跑 worker，保存 run 日志和截图 |
...
| BCG-07 | 运行日志与决策回放 | 运行页 / 任务详情 | `/tasks/{id}/decision-history`、run log API | run logging / decision replay 存在 | Backend Pass / Runtime Evidence Missing | 需用真实 run 验证日志、失败分类、质量门、策略回放完整 | 运行详情可能空或看似“调试页” | P1 证据 | 取成功/失败/阻断三类 run 做日志证据包 |
| BCG-08 | AI 运行摘要闭环 | 运行页摘要卡 | `/runs/{run_id}/ai-summary`、generate、regenerate | 独立 `run_ai_summaries`，AI 优先 + rule fallback；DeepSeek 运行时曾验收 | Backend Pass / Runtime Evidence Partial | 不是 P0 缺口；但需纳入总 Gate 当前环境重跑证据；2C-C 失败回退体验未开始 | 若不记录，会被误判为漏项或重复开发 | P0 证据 / P2 体验 | 总 Gate 重跑一次 source=ai；2C-C 做失败回退体验收口 |
| BCG-09 | 自动交付件与审批创建 | Worker 成功后 / `/delivery` | deliverables / approvals | Worker 成功后可自动创建运行交付快照与审批请求 | Backend Pass / Runtime Evidence Missing | 需真实 provider run 后确认交付件和审批是否自动生成 | 用户找不到运行结果进入审批的证据 | P1 证据 | 成功 run → deliverable → approval 自动链路验证 |
...
| ID | 任务 | 原因 |
|---|---|---|
| BCG-01 | AI Project Director 顶层会话 API | 当前 Agent thread 偏执行会话，不等于项目主管目标澄清 |
| BCG-02 | 计划版本与用户确认记录 | `planning/apply` 能生成任务，但确认审计层不足 |
| BCG-03 | 待确认事项聚合 inbox | 决策点分散在计划、预检、审批、人工处理等模块 |
...
| BCG-08 | AI 运行摘要 | 当前环境重新生成一次 `source=ai` 摘要，保存 GET/POST 证据 |
...
| BCG-07 | 成功/失败/阻断 run 日志样例 | 主要是证据与体验 |
| BCG-09 | 自动交付件与审批证据 | 有助于交付闭环验收 |
...
4. BCG-21：Skill Consumption Evidence 只读接口

说明：这四个最能解决“AI 项目主管不像主管”的问题。
