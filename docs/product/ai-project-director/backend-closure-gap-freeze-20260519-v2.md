...
| BCG-11 | 仓库绑定与快照 | 设置页 / 仓库工作区 | workspace settings、bind repo、snapshot refresh | 后端存在，安全边界存在；2026-05-24 BCG-11A live evidence: sample Git repo bound, snapshot refreshed with tree/language/ignore rules, safety boundary verified (57/57 checks passed) | Backend Pass / Runtime Evidence Pass for repository binding/snapshot | 页面空不等于没后端；BCG-11A 已完成绑定+快照+安全边界+读回证据 | P0 证据 / P0 收口 | BCG-11A 已完成 repository binding + snapshot evidence；后续接 BCG-12 file locator / context pack |
