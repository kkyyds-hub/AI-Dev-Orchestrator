...
| BCG-14 | 预检与人工放行 | 仓库工作区 / 审批 | preflight、repository-preflight actions | BCG-14A live evidence: preflight (wide_change + dangerous commands), manual approve (ready_for_execution), illegal-action protection (422/404), inbox/day15-flow read-back; 74/74 passed; 1 gap (manual reject not tested due to active batch conflict) | Backend Pass / Runtime Evidence Partial | Manual reject scenario blocked by active-batch constraint (one active batch per project). Preflight classification, approve, read-back, and illegal-action protection all verified. Reject shares the same service path as approve — only the state differs. | 危险变更被正确识别并阻断；放行后可继续执行 | P1 证据 | 后续可创建 fresh project 专门验证 reject；接 BCG-15 提交草案 |
