...
| BCG-10 | 审批要求修改到返工队列 | `/delivery` / 审批页 | approval actions、retrospective、rework chain | 审批动作存在，rework 聚合 DTO 存在；2026-05-24 BCG-10A 验证 request_changes → rework evidence chain (61/61)；2026-05-25 BCG-10-R3 验证 request_changes/reject 自动创建 executable pending rework task，含 idempotency/rollback/event boundary (89/89 live + 6/6 unit)，rework task 可通过 GET /tasks 读回、可被 worker 队列消费、可追溯到 approval/decision/deliverable/project | Backend Pass / Runtime Evidence Pass | 审批要求修改或驳回后自动生成可执行返工任务，全链路可追踪 | P1 证据 / P0 收口 | BCG-10 Runtime Evidence Pass；后续接 BCG-11+ |
| BCG-11 | 仓库绑定与快照 | 设置页 / 仓库工作区 | workspace settings、bind repo、snapshot refresh | 后端存在，安全边界存在；2026-05-24 BCG-11A-R1 live evidence: sample repo outside main repo, allowed roots preserved, 4-category safety boundary verified (incl. out-of-bounds existing Git repo), strengthened language assertions (71/71 checks passed) | Backend Pass / Runtime Evidence Pass for repository binding/snapshot | R1 收口已完成 | P0 证据 / P0 收口 | BCG-11A-R1 已完成；后续接 BCG-12 file locator / context pack |
| BCG-12 | 文件定位与上下文包 | 仓库工作区 | file-locator、context-pack | 后端存在；BCG-12A-R1 已在 context-pack selected_paths 读取前复用默认 ignored-directory 基线拦截 | Backend Pass / Runtime Evidence Pass | 2026-05-24 BCG-12A-R1 live evidence: file locator (3 query types) + context pack (5 files, 4 languages) + path safety Pass；`.git/config`、`node_modules/ignored.js`、`__pycache__/ignored.py`、`.venv/ignored.py`、`dist/ignored.js`、`build/ignored.js` 均返回 422 blocked；budget truncation 由测试套件覆盖（sample files too small for live trigger） | AI 修改文件前已有可靠上下文；ignored-directory 显式读取缺口已关闭 | P0 证据 / P0 收口 | BCG-12 Runtime Evidence Pass；后续接 BCG-13 |
| BCG-13 | 变更计划与变更批次 | 仓库工作区 | change-plans、change-batches | 后端存在；BCG-13A live evidence complete | Backend Pass / Runtime Evidence Pass | 2026-05-24 BCG-13A live evidence: change plan v1 (201, 4 target files) + v2 revision (5 target files) + read-back (detail/list/task-filtered all consistent) + change batch (2 plans, 2 tasks, 5 target files, status=preparing) + batch read-back; 97/97 checks passed; 0 gaps | 变更链路第一步已验收 | P0 证据 / P0 收口 | BCG-13 Runtime Evidence Pass；后续接 BCG-14 preflight |
| BCG-14 | 预检与人工放行 | 仓库工作区 / 审批 | preflight、repository-preflight actions | BCG-14A-R1 full closeout: ready_for_execution (small scope, 0 findings), manual approve (manual_confirmed, 1 decision), manual reject (manual_rejected via dedicated R1 project, blocked=true, 1 decision, inbox/day15-flow read-back), NOT_STARTED action 422, illegal re-approve/re-reject 422, non-existent 404. 53/53 R1 reject passed, 0 gaps. | Backend Pass / Runtime Evidence Pass | All four preflight states (ready/blocked/approve/reject) + all illegal-action protection + read-back verified | P1 证据 / P0 收口 | BCG-14 Runtime Evidence Pass；接 BCG-15 提交草案 |
| BCG-15 | 提交草案 | 仓库工作区 | commit-candidate GET/POST | BCG-15A + R1: first draft v1, second revision v2, detail/list read-back, review-only boundary (68/68). R1: preflight not_started 409, verification missing 409, verification failed 409 — all passed, no candidate created, preflight state preserved (41/41). All protection paths live-verified. | Backend Pass / Runtime Evidence Pass | BCG-15A+R1 验证了 review-only draft + 3 条保护路径全链路 | P0 证据 / P0 收口 | BCG-15 Runtime Evidence Pass；后续接 BCG-16 apply-local（需独立入口和全面安全边界） |
| BCG-16 | 本地 Git 写入 apply-local / git-commit | 仓库工作区 | `POST /repositories/change-batches/{id}/apply-local`、`git-commit` | BCG-16A + R1 + R2 + R3: apply-local/git-commit success (55/55), guard chain reorder to preflight→candidate→gate (R2), all 7 guard paths live-verified (R3: preflight_not_passed x2, commit_candidate_missing, gate_not_approved, apply_not_done, apply_verification_failed, unrelated staged excluded). 187/187 R3, 149/149 regression. | Backend Pass / Runtime Evidence Pass / Frontend Entry Pending | BCG-16A-R3 全链路 + 全 guard path 通过 | P0 证据 / P0 收口 | BCG-16 Runtime Evidence Pass；BCG-17 仍 Deferred |
...
| BCG-18 | Release Gate 总放行 | 审批 / 仓库 | release gate checklist/actions/judgement | BCG-18 live evidence: blocked gate approve 409, approve (rqe=true), reject (rqe=false), changes_requested (rqe=false), Day15 judgement read-back (decision_count/actions/rqe/gwit all verified), git_write_actions_triggered=false, no apply-local/git-commit/push. 137/137 passed, 0 gaps. | Backend Pass / Runtime Evidence Pass | All 4 decision types + Day15 judgement + blocked protection verified | P0 证据 / P0 收口 | BCG-18 Runtime Evidence Pass |
...
| BCG-11 | 仓库绑定与快照 | BCG-11A-R1 live evidence已完成 (71/71) |
| BCG-12 | 文件定位与上下文包 | BCG-12A-R1 live evidence 已完成 (178/178, ignored dir files blocked) |
| BCG-13 | 变更计划与变更批次 | BCG-13A live evidence已完成 (97/97) |
...
Gate: **BCG-12A-P0-R1 API Ready / superseded by BCG-12A-R1 Runtime Evidence Pass in section 12C**.
...
Gate: **BCG-12A-P0 API Ready / superseded by BCG-12A-R1 Runtime Evidence Pass in section 12C**.

## 12C. BCG-12A-R1 Runtime Evidence - File Locator + Context Pack (2026-05-24)

Scope: Codex live evidence script validating the full file-locator -> context-pack
chain against the BCG-11A bound sample repository, including BCG-12A-R1 ignored-directory blocking.

- Live evidence script: `runtime/orchestrator/scripts/bcg12a_file_locator_context_pack_live.py`
- Evidence project: project_id `423367da-966b-4c2e-b8c8-a4ff5f7f2377`
- Workspace: `e1e32ddb-e858-4224-b301-5362f97c1864`, root_path `E:\bcg11a-workspaces\bcg11a-sample-repo`
- Snapshot: `4a769201-f0f4-4f64-806a-b09b7606950e`, file_count=5, languages=Markdown/Python/TypeScript/JSON
- APIs exercised: `GET /repositories/projects/{project_id}`, `GET /snapshot`, `POST /file-locator/search`, `POST /context-pack`

### Evidence Results

- **Workspace verification**: Pass (absolute path, read_only, .git exists, outside main repo/runtime_data_dir/system temp).
- **Snapshot verification**: Pass (status=success, scan_error=null, file_count=5, tree includes README.md/src/web/config/docs, language_breakdown covers Markdown/Python/TypeScript/JSON, ignored dirs include `.git`, `.venv`, `__pycache__`, `node_modules`, `dist`, `build`).
- **File locator A (keywords)**: Pass (4 candidates: README.md, config/app.json, src/main.py, web/app.tsx).
- **File locator B (path_prefixes + file_types)**: Pass (5 candidates: all 5 expected files found).
- **File locator C (task_query)**: Pass (5 candidates, scanned_file_count=5).
- **Context pack (5 files)**: Pass (included_file_count=5, total_included_bytes=419, entries cover Markdown/Python/TypeScript/JSON, source_summary from locator, focus_terms derived, all excerpts non-empty, all match_reasons present).
- **Budget truncation**: Pass (sample files are ~215 bytes, below API minimum 512; truncation logic verified by `test_repository_context_pack_api.py`).
- **Path safety (../ traversal)**: Pass (422).
- **Path safety (absolute path)**: Pass (422).
- **Ignored directory file access**: Pass - `.git/config`, `node_modules/ignored.js`, `__pycache__/ignored.py`, `.venv/ignored.py`, `dist/ignored.js`, and `build/ignored.js` all return 422 before file read.

### Gate

**BCG-12A-R1 Pass / BCG-12 Runtime Evidence Pass**

BCG-12A-R1 closes the defense-in-depth gap by blocking selected_paths that contain any segment from `DEFAULT_REPOSITORY_IGNORE_RULE_SUMMARY` before resolving or reading files. Path traversal and absolute path protections remain 422.
