...
| BCG-10 | 审批要求修改到返工队列 | `/delivery` / 审批页 | approval actions、retrospective、rework chain | 审批动作存在，rework 聚合 DTO 存在；2026-05-24 BCG-10A 验证 request_changes → rework evidence chain: approval history rework_required + change-rework snapshot visible (61/61 checks passed); gap: executable rework task not auto-created | Partial（approval rework evidence Pass / executable rework task Missing） | 审批要求修改后在 history 和 change-rework 中可见，但无自动创建的可执行返工任务 | P1 | BCG-10A 完成 approval rework evidence；后续补 executable rework task creation |
