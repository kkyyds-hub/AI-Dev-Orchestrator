...
| CL-05 | 团队闭环 | 是否生成角色与 Skill 方案 | R1-N: GET /roles/catalog → 4 system roles (product_manager, architect, engineer, reviewer, each with responsibilities/default_skill_slots/boundaries)；GET /roles/projects/{pid} → 4 project instances auto-initialized；PUT customize instance (name/skills/notes)；GET /skills/registry → 12 skill templates；GET /skills/projects/{pid}/bindings → 4 role×3 binding groups；Worker dispatch uses owner_role_code+selected_skill_codes from config（verification-project-director-role-skill-team-governance-r1n-20260531） | 有角色、职责、Skill、边界 | Runtime Pass | 系统目录→项目实例→Worker 消费全链路 live HTTP 验证；GovernancePage 双视图展示 |
| CL-06 | 团队闭环 | 角色 / Skill 是否区分模板与项目实例 | R1-N: System template: id-less, no project_id, immutable (name=架构师, 3 skills)；Project instance: UUID + project_id, customizable (name=Custom Architect, 4 skills + custom_notes)；lifecycle labels: project_local / template_candidate / template_stable；PUT customization confined to instance only, template unchanged（verification-project-director-role-skill-team-governance-r1n-20260531） | 不混淆可复用资产和临时资产 | Runtime Pass | 模板 vs 实例在 API 和前端均区分；GovernancePage lifecycle tabs 按 project_local/template_candidate/template_stable 过滤 |
...
| CL-08 | 调度闭环 | 是否产生调度决策 | R1-Fb v3: export WORKER_SIMULATE_EXECUTION_OVERRIDE=1 → live HTTP POST /workers/run-once → execution_mode=simulate, claimed=True, dispatch_status=explicit_owner, route_reason 含 readiness/budget/stage/role, owner_role_code=architect, routing_score 存在（verification-project-director-worker-run-r1fb-20260529 v3） | 有 Agent 分配和原因 | Runtime Pass | v3 simulate-only live HTTP 验证通过；WORKER_SIMULATE_EXECUTION_OVERRIDE local evidence only；v1 provider_openai 已标记 Non-compliant |
| CL-09 | 运行闭环 | 是否产生 Run 记录 | R1-Fb v3: simulate Worker → run_id+run_status=succeeded; GET /tasks/{id}/runs → 200, 1 run, id_match; GET /runs/{run_id}/logs → 200; task status pending→completed（verification-project-director-worker-run-r1fb-20260529 v3） | 有状态、时间、关联任务 | Runtime Pass | v3 simulate-only live HTTP Run persist+readback+logs 完整 |
| CL-10 | 运行闭环 | Run 是否有摘要或 fallback | R1-Fb v3: GET /runs/{id}/decision-trace → 200; GET /runs/{id}/ai-summaries → 200; 40 tests (3 override + 37 summary) 全部通过（verification-project-director-worker-run-r1fb-20260529 v3） | source=ai 或 rule_fallback | Runtime Pass | v3 simulate-only decision-trace + ai-summaries readback 完整；summary service 40/40 tests |
| CL-11 | 失败闭环 | 失败/阻塞是否有下一步 | R1-G Runtime Pass: failed mode live HTTP (task_status=failed→retry→pending) + blocked mode live HTTP (task_status=blocked→request-human→waiting_human→resolve-human→pending) 全链路验证；failure-review 200（failed: execution_failed / blocked: retry_limit_exceeded）；decision-trace 200（12 trace items 含 failure_review）；16 tests 全通过（8 simulate + 2 evidence + 6 approval）；WORKER_SIMULATE_FAILURE_MODE 注入验证完成（verification-project-director-failure-closure-r1g-20260530） | 失败不死路 | Runtime Pass | failed + blocked 两组 live HTTP 全链路验证通过；retry/request-human/resolve-human 全部 readback 确认；不调 provider / worker pool / planning/apply / apply-local / git-commit |
| CL-12 | 仓库闭环 | 代码相关任务是否有仓库证据链 | R1-H: repository workspace binding (PUT+GET readback 200), snapshot refresh+readback (file_count=3, status=success), change session capture+readback (workspace_status=clean), file locator search (2 candidates), context pack build (2 files/35 bytes), day15 flow (2/9 steps, git_write_actions_triggered=False) 全部 live HTTP 验证；11 context pack tests 通过；CommitCandidate 明确为 "review-only draft"；BCL-03 apply-local/git-commit 为独立受控端点（verification-project-director-repository-evidence-r1h-20260530） | 不把草案伪装成真实 commit | Evidence Partial | 只读仓库证据链 live HTTP 完整通过；draft evidence 链后端完备（change plan→batch→preflight→commit candidate→release gate）但全端到端 live HTTP 需要 deliverables 前置；草案≠真实 commit 已确认 |
| CL-13 | 交付闭环 | 成功任务是否形成交付物 | R1-I: WORKER_SIMULATE_EXECUTION_OVERRIDE=1 → worker → task=completed, run=succeeded → auto-create deliverable (1 deliverable, type=stage_artifact) → GET /deliverables/projects/{pid} readback 确认 → version.source_task_id 匹配 task_id, version.source_run_id 匹配 run_id → GET /deliverables/tasks/{tid} 反向查找确认 → deliverable.id 可输入 ChangePlan.related_deliverable_ids（verification-project-director-deliverable-closure-r1i-20260530） | 有版本和来源证据 | Runtime Pass | simulate run → deliverable auto-create 全链路 live HTTP 验证；source_task_id / source_run_id / project_id 全部关联；163 tests pass |
| CL-14 | 审批闭环 | 交付物是否经过审批决策 | R1-J: WORKER_SIMULATE_EXECUTION_OVERRIDE=1 → 2 worker runs → 2 approvals auto-created → POST /approvals/{id}/actions approve → status=approved, decision persisted → GET readback confirmed → POST request_changes → status=changes_requested, rework task auto-created (executable, pending, HIGH priority, source_draft_id) → re-apply to closed → 422 idempotency guard；6 approval rework tests pass（verification-project-director-approval-closure-r1j-20260530） | 通过/修改/驳回有记录 | Runtime Pass | approve + request_changes 全链路 live HTTP 验证；rework task 自动生成；reject 路径由 6 tests 覆盖；APV-01~10 前端已验收 |
| CL-15 | 治理闭环 | 角色/Skill 是否记录消费证据 | R1-K Runtime Pass: Codex 补齐 GET /roles/projects/{pid}/consumption 聚合 API (role_code/run_count/succeeded/failed/total_tokens/estimated_cost/latest_run + skill_code/skill_name/run_count/latest_owner_role_code)；前端 GovernancePage TeamTab/RolesTab/SkillsTab 全接入 useProjectRoleSkillConsumption；3 tests (with runs/empty/404) pass；live HTTP 3 worker runs → consumption API 聚合 architect×2 + reviewer×1 + 6 skills；前端 build 14.74s；"待接入真实运行时消费证据"/"暂无消费证据" 已替换（verification-project-director-role-skill-consumption-r1k-20260530 v2） | 不是只保存配置 | Runtime Pass | Worker→Run→consumption API→GovernancePage 全链路闭合；3 backend tests + live HTTP + frontend build 通过 |
| CL-16 | 成本闭环 | AI 生成是否记录成本台账 | R1-L: WORKER_SIMULATE_EXECUTION_OVERRIDE=1 → Worker: total_tokens=1036/estimated_cost=$0.002084/model_name=deepseek-v4-pro；Run: token_pricing_source=heuristic.simulate.char_count.v1, token_accounting_mode=heuristic, cache_hit=false；Cost Dashboard API: run_count=6, total_cost=$0.010597, mode_breakdown=heuristic×6, role_breakdown=3 roles, fallback_contract fallback_active=True/provider_reported=0, budget_policy_source=not_configured；GovernancePage CostMemoryTab: real API call, displays cost/run/token, source credibility note; 37 tests pass; frontend build 3.62s（verification-project-director-cost-ledger-r1l-20260531） | 有模型、来源、缓存、成本 | Evidence Partial | Worker→Run→Cost Dashboard→GovernancePage 成本结构全链路闭合（live HTTP + frontend build）；所有成本为 heuristic（simulate 模式，$0.002/run）；真实 provider 成本需用户确认后验证；之前 provider_openai 证据 Non-compliant |
| CL-17 | 页面闭环 | 页面按钮是否真实闭环 | R1-O: 全站 70 按钮/入口审计（10 页面）：37 API + 21 Navigation + 2 Display + 5 Disabled with reason + 5 Manual = 0 Gap；工作台 (7 API) / 执行中心任务 (5 API) / 审批 (4 API) / 设置 save+test (2 API) / 治理中心 (7 API) 全部真实；高风险动作全部受控（worker pool/planning/apply/apply-local/git-commit/memory 按钮）；前端 build 3.64s（verification-project-director-fullsite-button-closure-r1o-20260531） | 无假按钮 | Runtime Pass（全站） | 全站 10 页面 70 入口 0 Gap；无假按钮；所有 disabled 按钮均有明确原因；CL-17 从工作台升级为全站 Runtime Pass |
| CL-18 | 文档闭环 | 产品文档是否同步更新 | R1-M: 全量 evidence/checklist/ledger 一致性审计通过；15/18 CL 有明确状态和 evidence 文档（另有 3 项已标注 Evidence Partial）；15 份 evidence (R1-A~R1-O) 覆盖所有已审计项；无文档冲突、无 simulate→provider 越界表述、无 total closure→Pass 越界表述；page-information-architecture 仍是主产品基线，未被覆盖（verification-project-director-total-gate-r1m-20260531） | 变更有文档依据 | Documentation Pass | checkpoint/ledger 持续回填；主基线未被覆盖；CL-05/CL-06/CL-17 已 Runtime Pass；CL-12/CL-16 Evidence Partial 未越界；total closure 仍为 Partial |
