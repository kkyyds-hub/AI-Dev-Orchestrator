...
| CL-15 | 治理闭环 | 角色/Skill 是否记录消费证据 | R1-Fb (corrected): pytest Worker response 结构含 owner_role_code+selected_skill_codes；v1 live HTTP provider_openai 消费数据越界已标记 Non-compliant（verification-project-director-worker-run-r1fb-20260529 v2） | 不是只保存配置 | Evidence Partial | Worker role/skill 结构已验证 (pytest)；治理中心端到端消费证据展示尚未接入；provider_openai 证据 Non-compliant |
