...
| CL-11 | 失败闭环 | 失败/阻塞是否有下一步 | R1-G: 后端 retry(FAILED/BLOCKED→PENDING) / pause / resume / request-human(PENDING/FAILED/BLOCKED→WAITING_HUMAN) / resolve-human(WAITING_HUMAN→PENDING) 完整；state machine guards → 409 验证；approval rework 6 tests + run evidence replay 1 test = 7 passed；GET /runs/{id}/failure-review → 200；GET /runs/{id}/decision-trace → 200（verification-project-director-failure-closure-r1g-20260530） | 失败不死路 | Evidence Partial | Backend pathways complete + verified；simulate-only live HTTP cannot produce failed task → Runtime Evidence Gap for end-to-end failure chain；frontend task action buttons exist (TASK-06~12) |
