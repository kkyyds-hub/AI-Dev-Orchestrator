...
| CL-16 | 成本闭环 | AI 生成是否记录成本台账 | R1-Fb (corrected): pytest Worker response 结构含 total_tokens+estimated_cost+provider_receipt；v1 live HTTP provider_openai cost 数据越界已标记 Non-compliant（verification-project-director-worker-run-r1fb-20260529 v2） | 有模型、来源、缓存、成本 | Evidence Partial | Worker cost 结构已验证 (pytest)；治理中心成本台账前端展示仍为静态；provider_openai 证据 Non-compliant |
