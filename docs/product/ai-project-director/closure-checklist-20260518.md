...
| CL-15 | 治理闭环 | 角色/Skill 是否记录消费证据 | R1-K: WORKER_SIMULATE_EXECUTION_OVERRIDE=1 → live HTTP worker response: owner_role_code=architect, selected_skill_codes=["dependency_analysis","solution_design","risk_assessment"]；decision-trace: role_handoff event (product_manager→architect→engineer) + task_routed with skills；task/runs API: strategy_decision.selected_skill_codes present；3 readback 路径数据一致；前端治理中心: "暂无消费证据" + "待接入真实运行时消费证据"（verification-project-director-role-skill-consumption-r1k-20260530） | 不是只保存配置 | Evidence Partial | Worker→Run→Readback 角色/Skill 消费链路完整（live HTTP）；治理中心前端仍为静态配置，需 governance 聚合 API + 前端消费展示补丁 |
