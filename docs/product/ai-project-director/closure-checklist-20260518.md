...
| CL-08 | 调度闭环 | 是否产生调度决策 | R1-Fb (corrected): pytest `test_project_director_worker_run_evidence.py` 证明 simulate Worker dispatch 链路完整；v1 live HTTP provider_openai 证据越界已标记 Non-compliant；simulate-only live HTTP not feasible via API-only path（verification-project-director-worker-run-r1fb-20260529 v2） | 有 Agent 分配和原因 | Evidence Partial | pytest simulate dispatch OK；simulate-only live HTTP gap。**不得写 Pass** |
| CL-09 | 运行闭环 | 是否产生 Run 记录 | R1-Fb (corrected): pytest 证明 simulate Run create+persist 完整；v1 live HTTP provider_openai Run 存在但越界；simulate-only live HTTP not feasible（verification-project-director-worker-run-r1fb-20260529 v2） | 有状态、时间、关联任务 | Evidence Partial | pytest simulate Run OK；simulate-only live HTTP gap。**不得写 Pass** |
| CL-10 | 运行闭环 | Run 是否有摘要或 fallback | R1-Fb (corrected): run_ai_summaries 37 tests 覆盖 L1/L2/L3 + rule_fallback + AI source；v1 Run readback 有 summary 但越界（verification-project-director-worker-run-r1fb-20260529 v2） | source=ai 或 rule_fallback | Evidence Partial | pytest summary 全路径 OK；simulate-only live HTTP gap。**不得写 Pass** |
