...
| ID | 验收项 | 回填证据 | 通过标准 | 状态 | 备注 |
|---|---|---|---|---|---|
| TASK-01 | 任务是否按调度优先级分组 | 截图/排序逻辑 | 待人工/阻塞、执行中、可调度、等待依赖、已完成 |  |  |
| TASK-02 | 任务列表是否弱化表格感 | 截图 | 轻列表，不是数据库表 |  |  |
| TASK-03 | 每个任务是否只展示核心 6 项 | 截图 | 标题、状态、Agent、优先级、阻塞/依赖、最近运行 |  |  |
| TASK-04 | 右侧是否为执行态势面板 | 截图 | Agent 负载、阻塞原因、最近运行、AI 建议 |  |  |
| TASK-05 | 任务详情是否用抽屉 | 截图 | 不做常驻大右栏 |  |  |
| TASK-06 | 暂停是否真实调用后端 | API 记录 | 状态改变 |  |  |
| TASK-07 | 恢复是否真实调用后端 | API 记录 | 状态改变 |  |  |
| TASK-08 | 请求人工是否真实调用后端 | API 记录 | waiting_human 状态 |  |  |
| TASK-09 | 人工已处理是否真实调用后端 | API 记录 | 状态回到可调度 |  |  |
| TASK-10 | 重新入队文案是否准确 | 截图 | 说明“下一次调度执行”，不等于立即运行 |  |  |
| TASK-11 | 查看运行是否跳运行观测 | 路由 | 带 runId/taskId |  |  |
| TASK-12 | 查看仓库上下文是否跳仓库工作区 | 路由 | 带 taskId/projectId |  |  |
| TASK-13 | 任务页是否不展示完整日志 | 截图 | 日志去运行观测 |  |  |
| TASK-14 | 任务页是否不展示仓库树 | 截图 | 仓库树去仓库弹窗 |  |  |
