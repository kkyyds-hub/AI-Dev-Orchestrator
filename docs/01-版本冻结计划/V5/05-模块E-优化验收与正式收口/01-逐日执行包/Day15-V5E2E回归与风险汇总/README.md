...
- 当前不要误判为完成：`Day15` 的 **verify 输出已完成**，但这不等于 `Day16 accept 已通过`。

## 2026-04-18 当前工作树复验附记

- 本附记基于 **当前未提交工作树** 复跑 Day15 verify，保留 2026-04-17 的历史正式结论，不把 verify 改写为 accept。
- 当前复验证据根目录：
  - `runtime/orchestrator/tmp/day15-verify-20260418/day15_verify_evidence.json`
- 当前复验新增事实：
  1. Day08 稳定入口 4 条继续通过（Day06 step2 / step6 / step10 + Day07 runtime pack）。
  2. Day09/Day10 `memory/governance` 的 `GET + rehydrate + compact + reset` 在隔离 runtime 数据目录下复跑通过。
  3. Day11/Day12 `workers/run-once + sessions/timeline/interventions` 在当前代码上复跑通过。
  4. Day13/Day14 `team-control-center` 与 `cost-dashboard` 在当前代码上复跑通过。
  5. `fallback_contract` 已在当前复验中拿到 `provider_reported / heuristic / missing` 三态消费事实；其中 `missing` 仍属于隔离 replay 证据，不等于自然执行链路自动产出。
  6. `GET /projects/{project_id}/day15-repository-flow` 对新建项目返回 `blocked` 摘要，属于 fresh project 缺少仓库闭环前置条件的事实，不应误判为接口故障。
  7. 前端 `npm.cmd run build` 继续通过；`npm run build` 继续受 PowerShell `ExecutionPolicy` 阻塞。
