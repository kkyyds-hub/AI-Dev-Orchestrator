...
> **状态**: P4-C Closure: Pass；P4-D Design R1: Design only；AI Project Director 总闭环 Partial
...
def evaluate(
    *,
    ...
    delivery_audit_event_present: bool | None = None,    # E6
    delivery_audit_event_type: str | None = None,        # E6
    delivery_audit_event_ready: bool | None = None,      # E6
    delivery_git_write_enabled: bool = False,     # E8
) -> DeliveryGateEvidenceResult:
```

**E6 audit evidence 参数说明**：

| 参数 | 类型 | 含义 | 调用方职责 |
|------|------|------|----------|
| `delivery_audit_event_present` | `bool \| None` | 是否存在有效的 delivery audit event（AgentMessage timeline 中至少有一条 `delivery_diff_dry_run_*` 记录） | 调用方在调用前查询 AgentMessage timeline |
| `delivery_audit_event_type` | `str \| None` | 最近一条 delivery audit event 的 `event_type` 值 | 调用方从匹配的 AgentMessage 读取 |
| `delivery_audit_event_ready` | `bool \| None` | 最近一条 delivery audit event 是否为成功事件（`delivery_diff_dry_run_collected`） | 调用方从 content_detail JSON 解析 |

**关键设计约束**：
- builder 自身**不查询数据库**、不访问 `AgentMessageRepository`、不调用 `DeliveryEventAuditService`
- audit evidence 由调用方（如 `TaskWorker`）在调用 `evaluate()` 前从 AgentMessage timeline 读取，并以纯值形式传入
- 这保持了 P4-D 域模型的纯计算边界——builder 只做条件判断，不做 I/O
...
| G21 | delivery_audit_event_present=True | E6 | 存在有效的 delivery audit event（AgentMessage timeline 中至少有一条 `delivery_diff_dry_run_collected` 记录，且无 `delivery_diff_dry_run_failed` 记录） |
...
| `audit_evidence_missing` | G21 | 缺少交付审计记录，无法进行交付前检查。 |
...
**E6 audit evidence 参数合同**：

P4-D 的 `evaluate()` 接收三个 audit evidence 参数（`delivery_audit_event_present`、`delivery_audit_event_type`、`delivery_audit_event_ready`），它们都是简单值类型（`bool | None`、`str | None`）。builder 自身不查询 `AgentMessage` 表——这三个值由调用方在调用前通过 `DeliveryEventAuditService` 或直接查询 `AgentMessageRepository` 获取。

这种设计遵循与 `GitOperationDryRunBuilder.build_from_diff_evidence()` 相同的纯域模型模式：builder 接收已有数据，只做条件判断；调用方负责 I/O。
...
| 2 | 实现 `evaluate()` | 接收 AgentSession + GitDiffDryRunResult + GitOperationDryRunResult + delivery audit evidence 参数（3 个简单值）→ 交叉验证 21 个条件（含 G21 审计证据存在性） |
...
| 8 | 不在 builder 内查询数据库 | builder 不访问 `AgentMessageRepository`——audit evidence 由调用方以纯值传入 |
| 9 | 不跑全量 pytest | 只跑新增的 targeted tests |
| 10 | 不跑全量 build | 不触发 `apps/web build` |
