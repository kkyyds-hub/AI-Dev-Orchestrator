...
| Partial | 15 | worker run, run logs, AI summary, repository binding, snapshot, file locator, context pack, change plan, change batch, preflight, commit candidate, release gate, governance, cost telemetry, frontend integration |
| Missing | 1 | rollup |
| N/A | 1 | apply-local/git-commit（明确不在 scope） |

> 21 条主链 = 5 Pass + 15 Partial + 1 Missing。外加 1 条 N/A（apply-local/git-commit 显式排除），总计 22 条记录。
