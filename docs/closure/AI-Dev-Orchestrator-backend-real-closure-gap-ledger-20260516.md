...
| 2026-05-16 | BCL-04 | `de09e6b` | smoke 6/6 passed; compileall clean | Pass（初版，需返工） | BudgetGuard 消费项目 budget_policy：hard_stop_enabled 时 project daily_budget_usd 覆盖默认值；evaluate_before_execution 新增 project_id 参数；cost dashboard 返回 budget_policy_source；worker run-once 传递 project_id；默认行为不受影响 |
| 2026-05-16 | BCL-04 返工 | `8b62cd9` | smoke 6/6 passed; compileall clean | Pass（返工，需修复重复分支） | 返工修复：1) per_run_budget_usd 硬消费 — estimated_run_cost > per_run 时 allowed=False + bg-project-per-run-budget；2) daily_budget 真实 BLOCKED 测试（seed run cost 超限）；3) worker run-once 真实路径阻断 + executor mock 未被调用；4) run log JSONL 含 budget_policy_source；5) hard_stop=false → project_team_control_soft；6) cost dashboard source 与 BudgetGuard 一致 |
...
| 2026-05-16 | BCL-06 | `6377ed6` | smoke 3/3 passed; compileall clean | Pass | 方案 A：missing 产品化为 legacy_missing 口径。fallback_contract 新增 legacy_missing_run_count/missing_source_note/missing_source_breakdown；旧 missing_mode_run_count 保留；正常 worker 写入 provider_reported/heuristic/provider_mock 不计入 legacy_missing |
| 2026-05-16 | BCL-08 | `34b38d0` | smoke 4/4 passed; compileall clean | Pass（初版，需返工） | 新增 v5_backend_closure_evidence_rollup.py 脚本；聚合 provider/project/worker_runs/team_control_budget/repository_git_write/cost_dashboard；输出 JSON 证据包；空 DB 不崩溃，返回 pass_ready=false + blockers；不触发 provider/git write |
| 2026-05-16 | BCL-08 返工 | `6a8c7c0` | smoke 7/7 passed; compileall clean | Pass（返工，需修复 receipt/gate） | 返工修复：1) pass_ready/blocks 一致：新增 no_provider_reported_run/no_successful_run/no_runs/missing_provider_receipt/diagnostics_blocked + safety net；2) project_diagnostics 聚合 BCL-02 service 输出 overall_status/blocking_reason_codes/next_actions；3) repository_git_write 读取 release gate 文件 + git write state + latest_commit_sha + evidence_files_read；4) smoke 新增 JSON 文件写出自检、repo field 校验、no_provider_reported 阻断 |
| 2026-05-16 | BCL-08 receipt/gate 返工 | `ee34bb2` | smoke 9/9 passed; compileall clean | Pass（返工） | 修正：1) real_run_receipt_exists 只依据 provider_receipt_id，不再被 cache_source 误置；2) diagnostics_unavailable → blocker；3) release_gate_unknown / release_gate_not_approved → blocker；4) smoke 新增 cache_visible_but_no_receipt + release_gate_unknown_is_blocker |
| 2026-05-16 | BCL-08 diag 适配修复 | `99d905b` | smoke 9/9 passed; rollup diagnostics_unavailable=0 | Pass | 修复 _build_project_diag 中 FailureReviewService 构造参数（run_repository→failure_review_repository+run_logging_service）；113 个 project diagnostics 全部正常输出 blocked + real blocking_reason_codes |
| 2026-05-16 | Backend closure gate acceptance | `043c6ce` (返工修正) | rollup pass_ready=false; 4 blockers | **Partial** | Gate 裁定：BCL-01~06 与 BCL-08 实现级 Pass；BCL-07 Deferred/P2；rollup pass_ready=false，gate 结论 Partial。详见 docs/closure/AI-Dev-Orchestrator-backend-real-closure-gate-acceptance-20260516.md |
