...
| 2026-05-16 | BCL-03 index 返工 | `f74bcae` | smoke 8/8 passed; compileall clean | Pass（返工） | 返工修复：1) git_commit 前执行 git reset -- . 清空 index，防止预先 staged 的无关文件被提交；2) 新增 git diff --cached --name-only 精确校验，staged 与 changed_files 不一致时返回 staged_files_mismatch 阻断；3) smoke 新增 pre-staged unrelated file 场景 |
