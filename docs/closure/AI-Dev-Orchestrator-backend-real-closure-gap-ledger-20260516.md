...
| 2026-05-16 | BCL-02 | `4a9386f` | smoke 3/3 passed; compileall clean | Pass | 新增 GET /projects/{project_id}/closure-diagnostics 只读诊断接口；聚合 provider/repository/task_runtime/governance 状态并返回 blocking_reason_codes + next_actions；复用现有 repository/service，不产生副作用 |
