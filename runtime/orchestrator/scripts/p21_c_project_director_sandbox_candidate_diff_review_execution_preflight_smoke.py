"""P21-C-H-A Project Director readonly review execution preflight smoke.

Uses isolated runtime data and does not start reviewers, call providers,
execute reviews, write files, apply patches, or perform Git writes.
"""
...
def _base_summary(rd: Path, db: Path) -> dict[str, Any]:
    return {
        ...
        "p21_c_review_execution_preflight": None,
        "source_handoff_verified": False,
        "source_diff_verified": False,
        "source_diff_sha256": "",
        "source_diff_sha256_length": 0,
        "review_prompt_sha256": "",
        "review_prompt_sha256_length": 0,
        "review_prompt_bytes": 0,
        "review_input_schema_version": "",
        "review_output_schema_version": "",
        "review_scope_paths": [],
        "p21_c_h_a_user_confirmed_blocked": None,
        "p21_c_h_a_non_handoff_source_blocked": None,
        "p21_c_h_a_task_mismatch_blocked": None,
        "p21_c_h_a_diff_sha_mismatch_blocked": None,
        "p21_c_h_a_scope_mismatch_blocked": None,
...
def _chain(*, summary: dict[str, Any]) -> tuple[str, str]:
    ...
    with TestClient(app) as c:
        proj = _req(c, "POST", "/projects", 201,
            json={"name": "P21-C-H-A smoke", "summary": "P21-C-H-A smoke.", "status": "active", "stage": "execution"})
        sess = _req(c, "POST", "/project-director/sessions", 201,
            json={"project_id": proj["id"], "goal_text": "P21-C-H-A smoke"})
        sid = sess["id"]
            ...
            json={"source_task_id": tid, "source_message_id": p17,
                  ...
                  "file_operations": [
                      {"path": "runtime/orchestrator/app/domain/new_module.py", "operation": "create",
                       "reason": "P21-C-H-A smoke", "patch_preview": ["PREVIEW ONLY: no repository file was modified."]}]})
        ...
        handoff = _req(c, "POST", f"/project-director/sessions/{sid}/sandbox-candidate-diff-review-handoff", 200,
            json={"source_task_id": tid, "source_message_id": diff["message"]["id"], "user_confirmed": True})
        summary["p21_c_candidate_diff_review_handoff"] = handoff.get("review_handoff_status")
        handoff_msg_id = handoff["message"]["id"]

        # P21-C-H-A preflight
        preflight = _req(c, "POST",
            f"/project-director/sessions/{sid}/sandbox-candidate-diff-review-execution-preflight", 200,
            json={"source_task_id": tid, "source_message_id": handoff_msg_id, "user_confirmed": True})
        summary["p21_c_review_execution_preflight"] = preflight.get("review_execution_preflight_status")
        summary["source_handoff_verified"] = preflight.get("source_handoff_verified", False)
        summary["source_diff_verified"] = preflight.get("source_diff_verified", False)
        summary["source_diff_sha256"] = preflight.get("source_diff_sha256", "")
        summary["source_diff_sha256_length"] = len(preflight.get("source_diff_sha256", ""))
        summary["review_prompt_sha256"] = preflight.get("review_prompt_sha256", "")
        summary["review_prompt_sha256_length"] = len(preflight.get("review_prompt_sha256", ""))
        summary["review_prompt_bytes"] = preflight.get("review_prompt_bytes", 0)
        summary["review_input_schema_version"] = preflight.get("review_input_schema_version", "")
        summary["review_output_schema_version"] = preflight.get("review_output_schema_version", "")
        summary["review_scope_paths"] = preflight.get("review_scope_paths", [])
        summary["reviewer_started"] = preflight.get("reviewer_started", False)
        summary["review_executed"] = preflight.get("review_executed", False)
        summary["review_findings_generated"] = preflight.get("review_findings_generated", False)
        summary["review_verdict_generated"] = preflight.get("review_verdict_generated", False)
        summary["provider_called"] = preflight.get("provider_called", False)
        summary["native_executor_started"] = preflight.get("native_executor_started", False)
        summary["codex_started"] = preflight.get("codex_started", False)
        summary["claude_code_started"] = preflight.get("claude_code_started", False)
        summary["main_project_file_written"] = preflight.get("main_project_file_written", False)
        summary["sandbox_file_written"] = preflight.get("sandbox_file_written", False)
        summary["manifest_file_written"] = preflight.get("manifest_file_written", False)
        summary["diff_file_written"] = preflight.get("diff_file_written", False)
        summary["patch_applied"] = preflight.get("patch_applied", False)
        summary["worktree_created"] = preflight.get("worktree_created", False)
        summary["git_write_performed"] = preflight.get("git_write_performed", False)
        summary["worker_started"] = preflight.get("worker_started", False)
        summary["task_created"] = preflight.get("task_created", False)
        summary["run_created"] = preflight.get("run_created", False)

        # Blocked: user_confirmed=false
        _req(c, "POST",
            f"/project-director/sessions/{sid}/sandbox-candidate-diff-review-execution-preflight", 409,
            json={"source_task_id": tid, "source_message_id": handoff_msg_id, "user_confirmed": False})
        summary["p21_c_h_a_user_confirmed_blocked"] = "blocked"

        # Blocked: non-G source
        _req(c, "POST",
            f"/project-director/sessions/{sid}/sandbox-candidate-diff-review-execution-preflight", 409,
            json={"source_task_id": tid, "source_message_id": ce["message"]["id"], "user_confirmed": True})
        summary["p21_c_h_a_non_handoff_source_blocked"] = "blocked"

        # Blocked: task mismatch
        p11b = _req(c, "POST", f"/project-director/sessions/{sid}/evidence-to-agent/dry-run", 200,
            json={"user_goal": "P21-C-H-A second evidence"})
        ...
        _req(c, "POST",
            f"/project-director/sessions/{sid}/sandbox-candidate-diff-review-execution-preflight", 409,
            json={"source_task_id": tid_b, "source_message_id": handoff_msg_id, "user_confirmed": True})
        summary["p21_c_h_a_task_mismatch_blocked"] = "blocked"

        # SHA mismatch and scope mismatch are covered by contract tests
        summary["p21_c_h_a_diff_sha_mismatch_blocked"] = "covered_by_contract"
        summary["p21_c_h_a_scope_mismatch_blocked"] = "covered_by_contract"

        # Message readback
        msgs = _req(c, "GET", f"/project-director/sessions/{sid}/messages", 200)
        messages = msgs.get("messages") or []
        has_preflight = any(
            m.get("source_detail") == "p21_c_sandbox_candidate_diff_review_execution_preflight_ready"
            for m in messages
        )
        summary["p21_c_message_readback"] = "passed" if has_preflight else "failed"
...
def run_smoke(rd: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    required = (
        ...
        "p21_c_review_execution_preflight",
        "p21_c_h_a_user_confirmed_blocked", "p21_c_h_a_non_handoff_source_blocked",
        "p21_c_h_a_task_mismatch_blocked", "p21_c_message_readback",
        "isolated_runtime_data", "source_handoff_verified", "source_diff_verified",
    )
...
def main() -> int:
    ...
    rd = (Path(tempfile.mkdtemp(prefix="p21-c-h-a-smoke-")) if temp else args.runtime_dir).resolve()
    ...
    if args.json:
        ...
        print(f"P21-C-H-A preflight smoke: {summary['smoke_status']}")
