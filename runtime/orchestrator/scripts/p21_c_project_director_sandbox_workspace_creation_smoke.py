"""P21-C-C Project Director sandbox workspace creation smoke.
...
def _base_summary(runtime_data_dir: Path, sqlite_db_path: Path) -> dict[str, Any]:
    return {
        ...
        "p21_c_workspace_create_second_call": None,
        "p21_c_workspace_create_user_confirmed_blocked": None,
        "p21_c_workspace_create_non_manifest_source_blocked": None,
        "p21_c_workspace_create_task_mismatch_blocked": None,
        "p21_c_message_readback": None,
        "isolated_runtime_data": False,
        "workspace_path": None,
        "workspace_exists": False,
        "workspace_is_dir": False,
        "workspace_empty_or_no_files_written": False,
...
def _prepare_project_director_chain(
    *, summary: dict[str, Any],
    ...
    """Build full chain through P21-C-C.
    ...
    with TestClient(app) as client:
        project_payload = _request_json(
            ...
            json={"name": "P21-C-C smoke", "summary": "P21-C-C smoke test.", "status": "active", "stage": "execution"},
        )
        project_id = project_payload["id"]

        session_payload = _request_json(
            client, "POST", "/project-director/sessions", 201,
            json={"project_id": project_id, "goal_text": "P21-C-C workspace creation smoke"},
        )
        ...
        p11_payload = _request_json(
            ...
            json={"user_goal": "P21-C-C evidence"},
        )
        ...
        # P21-C-C workspace create (first call)
        ...
        summary["workspace_path"] = create_payload.get("workspace_path")
        summary["workspace_created"] = create_payload.get("workspace_created", False)
        summary["workspace_already_existed"] = create_payload.get("workspace_already_existed", False)
        summary["workspace_written"] = create_payload.get("workspace_written", False)
        summary["file_written"] = create_payload.get("file_written", False)
        summary["manifest_file_written"] = create_payload.get("manifest_file_written", False)
        summary["target_file_content_read"] = create_payload.get("target_file_content_read", False)
        summary["real_diff_generated"] = create_payload.get("real_diff_generated", False)
        summary["patch_applied"] = create_payload.get("patch_applied", False)
        summary["worktree_created"] = create_payload.get("worktree_created", False)
        summary["rollback_snapshot_created"] = create_payload.get("rollback_snapshot_created", False)
        summary["git_write_performed"] = create_payload.get("git_write_performed", False)
        summary["worker_started"] = create_payload.get("worker_started", False)
        summary["task_created"] = create_payload.get("task_created", False)
        summary["run_created"] = create_payload.get("run_created", False)
        summary["cleanup_required"] = create_payload.get("cleanup_required", False)

        # Verify workspace directory
        ws_path = Path(create_payload.get("workspace_path", ""))
        ...
        if ws_path.exists() and ws_path.is_dir():
            ...
            summary["workspace_empty_or_no_files_written"] = len(contents) == 0
        else:
            summary["workspace_empty_or_no_files_written"] = False

        # P21-C-C workspace create (second call - already_exists)
        create2_payload = _request_json(
            client, "POST", f"/project-director/sessions/{session_id}/sandbox-workspace-create", 200,
            json={
                "source_task_id": source_task_id, "source_message_id": manifest_message_id,
                "user_confirmed": True,
        ...
        summary["p21_c_workspace_create_second_call"] = create2_payload.get("creation_status")

        # Blocked: user_confirmed=false
        _request_json(
            client, "POST", f"/project-director/sessions/{session_id}/sandbox-workspace-create", 409,
            json={
                "source_task_id": source_task_id, "source_message_id": manifest_message_id,
                "user_confirmed": False,
            },
        )
        summary["p21_c_workspace_create_user_confirmed_blocked"] = "blocked"

        # Blocked: non-manifest source
        _request_json(
            client, "POST", f"/project-director/sessions/{session_id}/sandbox-workspace-create", 409,
            json={
                "source_task_id": source_task_id, "source_message_id": guard_message_id,
        ...
        summary["p21_c_workspace_create_non_manifest_source_blocked"] = "blocked"

        # Blocked: task mismatch
        p11_b = _request_json(
            client, "POST", f"/project-director/sessions/{session_id}/evidence-to-agent/dry-run", 200,
            json={"user_goal": "P21-C-C second evidence"},
        )
        ...
        _request_json(
            client, "POST", f"/project-director/sessions/{session_id}/sandbox-workspace-create", 409,
            json={
                "source_task_id": task_b_id, "source_message_id": manifest_message_id,
                "user_confirmed": True,
            },
        )
        summary["p21_c_workspace_create_task_mismatch_blocked"] = "blocked"
        ...
        has_create = any(
            item.get("source_detail") == "p21_c_sandbox_workspace_created"
            for item in messages
        )
        summary["p21_c_message_readback"] = "passed" if has_create else "failed"
...
def run_smoke(runtime_data_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    required_checks = (
        ...
        "p21_c_workspace_create_second_call",
        "p21_c_workspace_create_user_confirmed_blocked",
        "p21_c_workspace_create_non_manifest_source_blocked",
        "p21_c_workspace_create_task_mismatch_blocked",
        "p21_c_message_readback",
        "isolated_runtime_data",
        "workspace_exists",
        "workspace_is_dir",
        "workspace_empty_or_no_files_written",
    )
...
def main() -> int:
    ...
    runtime_data_dir = (
        Path(tempfile.mkdtemp(prefix="p21-c-workspace-creation-smoke-"))
    ...
    if args.json:
        ...
        print(f"P21-C-C workspace creation smoke: {summary['smoke_status']}")
