...
def seed_base_records(
    ...
    *,
    ...
    session.add(
        ...
        risk_level="normal", human_status="none", owner_role_code="architect",
...
class FakeBoundedReworkExecutor:
    """Controlled external P25 executor that mutates only the declared workspace file."""

    def __init__(self) -> None:
        self.calls = 0

    def execute_bounded_rework(self, request):
        from app.external_executors.project_director_bounded_rework_executor import (
            ProjectDirectorBoundedReworkExecutorResult,
        )

        self.calls += 1
        target = Path(request.workspace_path) / "src/example.py"
        target.write_text("value = 'reworked'\n", encoding="utf-8")
        return ProjectDirectorBoundedReworkExecutorResult(
            declared_changed_paths=("src/example.py",),
            safe_summary="Updated the declared sandbox file.",
        )
...
def make_p25_auto_rework_d3_stack(
    ...
    *,
    ...
    evidence_diff_service = _P25EvidenceDiff(environment)
    evidence_handoff_service = _P25EvidenceHandoff(environment)
    p22_svc = _make_p22_service(
        session,
        msg_repo,
        sess_repo,
        task_repo,
        candidate_diff_service=evidence_diff_service,
        review_handoff_service=evidence_handoff_service,
    )
    initial_review_id = _seed_real_p20_p21_review_chain(
        msg_repo,
        session_id=session_id,
        task_id=task_id,
        project_id=project_id,
    ...
    assert initial_summary.result.orchestration_status == "ready_for_future_transition", (
        initial_summary.result.orchestration_status,
        initial_summary.result.blocked_reasons,
    )
    ...
    fake_bounded_executor = FakeBoundedReworkExecutor()

    def make_p25_execution_service():
        evidence_resolver = ProjectDirectorBoundedReworkEvidenceResolver(
            ...
            skill_binding_config_repository=(
                ProjectDirectorSkillBindingConfigRepository(session)
            ),
            verification_config_repository=(
                ProjectDirectorVerificationConfigRepository(session)
            ),
        ...
        outcome_service = ProjectDirectorBoundedReworkInvocationOutcomeService(
            ...
            bounded_rework_executor=fake_bounded_executor,
        )
        return ProjectDirectorBoundedReworkExecutionOrchestrationService(
            package_preparation_service=package_service,
            attempt_reservation_service=reservation_service,
            invocation_outcome_service=outcome_service,
        )

    p25_execution_svc = make_p25_execution_service()
    # The dedicated coordinator starts from the real initial P21-C authority.
    # The synthetic P25-H evidence above remains only for P23 convergence tests.
    preflight_result = preflight_svc.prepare_protected_transition_dispatch_consumption_preflight(
        session_id=session_id,
        source_task_id=task_id,
        source_message_id=prior_intent.message.id,
    )
    assert preflight_result.message is not None
    consumption_result = d1_svc.consume_protected_transition_dispatch_preflight(
        session_id=session_id,
        source_task_id=task_id,
        source_message_id=preflight_result.message.id,
    )
    assert consumption_result.message is not None
    ...
    d3_svc = ProjectDirectorProtectedTransitionAutoAdvanceService(
        ...
        worker_start_reservation_service=b1_svc,
        worker_invocation_service=b2_svc,
        bounded_rework_execution_orchestration_service=p25_execution_svc,
    )
    ...
    return {
        ...
        "b2_svc": b2_svc,
        "d3_svc": d3_svc,
        "p25_execution_svc": p25_execution_svc,
        "make_p25_execution_service": make_p25_execution_service,
        "fake_worker": fake_worker,
        "fake_bounded_executor": fake_bounded_executor,
        "session_id": session_id,
        "task_id": task_id,
        "project_id": project_id,
        ...
        "intent_result": prior_intent,
        "p22_summary": p22_summary,
        "candidate_diff_id": candidate_diff_id,
        "review_outcome_id": outcome.review_outcome_id,
        "consumption_message_id": consumption_result.message.id,
        "run_id": consumption_result.result.run_id,
        "environment": environment,
    }
