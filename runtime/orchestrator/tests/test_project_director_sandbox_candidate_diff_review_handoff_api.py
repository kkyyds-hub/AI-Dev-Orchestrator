"""API integration tests for P21-C-G readonly real diff review handoff."""
...
def _sf(tmp_path):
    db_path = tmp_path / "orchestrator-p21cg-test.db"
...
def _prepare_full_chain(client, sf):
    """Build full chain through P21-C-F. Returns (session_id, task_id, p21cf_msg_id)."""
    s = client.post("/project-director/sessions", json={"goal_text": "P21-C-G test"})
    ...
    ce = client.post(f"/project-director/sessions/{sid}/sandbox-candidate-files-write",
                     json={"source_task_id": tid, "source_message_id": write.json()["message"]["id"],
                           "user_confirmed": True,
                           "candidate_files": [
                               {"relative_path": "runtime/orchestrator/app/domain/new_module.py",
                                "content": "print('new')\n", "operation": "create"}]})
    ...
    return sid, tid, diff.json()["message"]["id"]
...
def test_handoff_success(tmp_path) -> None:
    sf = _sf(tmp_path)
    app = _app(sf)
    with TestClient(app) as client:
        sid, tid, diff_msg_id = _prepare_full_chain(client, sf)
        msgs_before = _count(sf, ProjectDirectorMessageTable)
        tasks_before = _count(sf, TaskTable)
        runs_before = _count(sf, RunTable)

        r = client.post(
            f"/project-director/sessions/{sid}/sandbox-candidate-diff-review-handoff",
            json={"source_task_id": tid, "source_message_id": diff_msg_id, "user_confirmed": True},
        )
        assert r.status_code == 200
        p = r.json()

        assert p["review_handoff_status"] == "created"
        assert p["source_diff_verified"] is True
        assert p["source_diff_message_bound"] is True
        assert len(p["source_diff_sha256"]) == 64
        assert p["diff_file_count"] >= 1
        assert p["diff_bytes"] > 0
        assert len(p["review_scope_paths"]) >= 1
        assert p["reviewer_started"] is False
        assert p["review_executed"] is False
        assert p["review_findings_generated"] is False
        assert p["review_verdict_generated"] is False
        assert p["main_project_file_written"] is False
        assert p["sandbox_file_written"] is False
        assert p["manifest_file_written"] is False
        assert p["diff_file_written"] is False
        assert p["patch_applied"] is False
        assert p["worktree_created"] is False
        ...
        assert p["message"]["source_detail"] == P21_C_SANDBOX_CANDIDATE_DIFF_REVIEW_HANDOFF_SOURCE_DETAIL
        actions = p["message"].get("suggested_actions") or []
        assert len(actions) >= 1
        assert actions[0]["type"] == "p21_c_sandbox_candidate_diff_review_handoff_record"
...
def test_handoff_user_confirmed_false_blocks(tmp_path) -> None:
    sf = _sf(tmp_path)
    app = _app(sf)
    with TestClient(app) as client:
        sid, tid, diff_msg_id = _prepare_full_chain(client, sf)
...
def test_handoff_non_diff_source_blocks(tmp_path) -> None:
    sf = _sf(tmp_path)
    app = _app(sf)
    with TestClient(app) as client:
...
def test_handoff_task_mismatch_blocks(tmp_path) -> None:
    sf = _sf(tmp_path)
    app = _app(sf)
    with TestClient(app) as client:
        sid, tid, diff_msg_id = _prepare_full_chain(client, sf)
        ...
        r = client.post(
            f"/project-director/sessions/{sid}/sandbox-candidate-diff-review-handoff",
            json={"source_task_id": str(uuid4()), "source_message_id": diff_msg_id, "user_confirmed": True},
        )
        assert r.status_code == 409
        assert _count(sf, ProjectDirectorMessageTable) == msgs_before
...
def test_message_readback(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        handoff_msgs = [m for m in msgs if m["source_detail"] == P21_C_SANDBOX_CANDIDATE_DIFF_REVIEW_HANDOFF_SOURCE_DETAIL]
        assert len(handoff_msgs) >= 1
        actions = handoff_msgs[0].get("suggested_actions") or []
        assert len(actions) >= 1
        assert actions[0]["type"] == "p21_c_sandbox_candidate_diff_review_handoff_record"
        assert len(actions[0]["source_diff_sha256"]) == 64

        serialized = json.dumps(handoff_msgs[0], ensure_ascii=False)
