"""Atomic P24-E3B exact Worker invocation claim persistence."""
...
_E3A_CONTENT_PREFIX = "P24 exact Worker invocation claim"
...
class _Blocked(Exception):
    def __init__(
        self,
        reason: CrossTaskExactWorkerInvocationClaimBlockedReason,
    ) -> None:
        self.reason = reason
        super().__init__(reason)


class ProjectDirectorCrossTaskExactWorkerInvocationClaimService:
    """Persist or replay one commit-before-call P24 invocation claim."""
    ...
    def claim_exact_worker_invocation(
        self,
        *,
        session_id: UUID,
        project_id: UUID,
        ...
        """Create, replay, or fail closed for the exact E2A authority."""
    ...
    def _claim_in_transaction(
        ...
        *,
        ...
        root, package, exact_run_reservation, worker_start_reservation = (
            self._locate_exact_graph(
                history=history,
                session_id=session_id,
                project_id=project_id,
                continuation_root_record_id=continuation_root_record_id,
        ...
        replay_key = (
            ProjectDirectorCrossTaskExactWorkerInvocationClaim
            .compute_worker_invocation_claim_replay_key(
                continuation_id=root.continuation_id,
                exact_worker_start_reservation_id=(
                    worker_start_reservation.exact_worker_start_reservation_id
                ),
        ...
        replay_matches = [
            item
            for item in history.invocation_claims
            if item[1].worker_invocation_claim_replay_key == replay_key
        ]
        if replay_matches:
            message, claim = replay_matches[0]
            return self._replay_existing_claim(
                message=message,
                claim=claim,
                root=root,
                package=package,
        ...
        if any(
            ...
            == worker_start_reservation.exact_worker_start_reservation_id
            or claim.exact_run_reservation_id
            == exact_run_reservation.exact_run_reservation_id
            or claim.exact_run_id == exact_run_reservation.exact_run_id
            for _, claim in history.invocation_claims
        ):
            raise _Blocked("exact_worker_invocation_claim_replay_conflict")

        task = self._load_exact_task(worker_start_reservation.next_task_id)
        self._validate_task(
            task=task,
            package=package,
            exact_run_reservation=exact_run_reservation,
        ...
        run = self._load_exact_run(worker_start_reservation.exact_run_id)
        self._validate_run(
            run=run,
            package=package,
            exact_run_reservation=exact_run_reservation,
        ...
        active_run_ids = self._active_run_ids(
            next_task_id=worker_start_reservation.next_task_id,
            exact_run_id=worker_start_reservation.exact_run_id,
        )
        active_agent_session_ids = self._active_agent_session_ids(
            worker_start_reservation.next_task_id
        )
        ...
        claim = self._build_claim(
            ...
            active_run_ids_before=active_run_ids,
            active_agent_session_ids_before=active_agent_session_ids,
            exact_worker_invocation_claim_id=uuid4(),
            created_at=utc_now(),
        )
        message = self._build_claim_message(
            claim=claim,
        ...
        self._post_write_validate(persisted_message, claim)
        return ProjectDirectorCrossTaskExactWorkerInvocationClaimResult(
    ...
    def _replay_existing_claim(
        ...
        *,
        ...
        expected = self._build_claim(
            root=root,
            package=package,
            exact_run_reservation=exact_run_reservation,
            worker_start_reservation=worker_start_reservation,
            task_human_status=claim.task_human_status_before,
            exact_run_started_at=claim.exact_run_started_at,
            exact_run_created_at=claim.exact_run_created_at,
            active_run_ids_before=claim.active_run_ids_before,
            active_agent_session_ids_before=(
                claim.active_agent_session_ids_before
            ),
            exact_worker_invocation_claim_id=(
                claim.exact_worker_invocation_claim_id
            ),
            created_at=claim.created_at,
            reason="exact_worker_invocation_claim_replay_conflict",
        )
        if claim != expected:
            raise _Blocked("exact_worker_invocation_claim_replay_conflict")
        self._validate_claim_message_binding(
            message,
            claim,
            reason="exact_worker_invocation_claim_replay_conflict",
        )
        ...
        *,
        ...
        for message in messages:
            family_flags = (
                ...
                self._is_invocation_claim_family(message),
            )
            ...
            if family_flags[0]:
                reason: CrossTaskExactWorkerInvocationClaimBlockedReason = (
                    ...
                    else "exact_worker_invocation_claim_history_invalid"
                )
                packages.append(
                ...
                reason = (
                    ...
                    if (
                        self._raw_action_id_matches(
                            message,
                            "exact_worker_start_reservation_id",
                            exact_worker_start_reservation_id,
                        )
                        or self._raw_action_id_matches(
                            message,
                            "exact_run_reservation_id",
                            exact_run_reservation_id,
                        )
                ...
                invocation_claims.append(
                    (
                        message,
                        self._parse_invocation_claim_message(
                            message,
                            reason=reason,
                        ),
                    )
        ...
        history = _History(
            ...
            invocation_claims=tuple(invocation_claims),
        )
    ...
    def _iter_session_messages(
        ...
        while True:
            ...
            if not has_more:
                break
            if not page:
                raise ValueError("invocation claim history page is empty")
            next_cursor = page[0].id
            if next_cursor == before_message_id or next_cursor in seen_cursors:
                raise ValueError("invocation claim history cursor stalled")
        ...
        if (
            ...
            raise ValueError("invocation claim sequence history is invalid")
        return ordered
    ...
    def _is_package_family(message: ProjectDirectorMessage) -> bool:
        return (
            ...
            or ProjectDirectorCrossTaskExactWorkerInvocationClaimService
    ...
    @staticmethod
    def _is_invocation_claim_family(
        message: ProjectDirectorMessage,
    ) -> bool:
        return (
            message.intent == CROSS_TASK_EXACT_WORKER_INVOCATION_CLAIM_INTENT
            or message.source_detail
            == CROSS_TASK_EXACT_WORKER_INVOCATION_CLAIM_SOURCE_DETAIL
            or ProjectDirectorCrossTaskExactWorkerInvocationClaimService
            ._has_action_type(
    ...
    def _parse_invocation_claim_message(
        self,
        message: ProjectDirectorMessage,
        *,
        reason: CrossTaskExactWorkerInvocationClaimBlockedReason,
        ...
        action = self._strict_action(
            ...
            action_type=CROSS_TASK_EXACT_WORKER_INVOCATION_CLAIM_ACTION_TYPE,
        ...
        payload.pop("type", None)
        try:
            claim = ProjectDirectorCrossTaskExactWorkerInvocationClaim.model_validate(
                payload
            )
        except (TypeError, ValueError, ValidationError) as exc:
            raise _Blocked(reason) from exc
    ...
    @classmethod
    def _validate_claim_message_binding(
        cls,
        message: ProjectDirectorMessage,
        claim: ProjectDirectorCrossTaskExactWorkerInvocationClaim,
        *,
        ...
        if (
            (action if action is not None else message.suggested_actions[0])
            != expected_action
            or message.id != claim.exact_worker_invocation_claim_id
            or message.content
            != (
                f"{_E3A_CONTENT_PREFIX}: "
                f"{claim.exact_worker_invocation_claim_id}"
            )
            or message.session_id != claim.session_id
            or message.related_plan_version_id != claim.plan_version_id
            or message.related_project_id != claim.project_id
            or message.related_task_id != claim.next_task_id
            or message.created_at != claim.created_at
            or message.forbidden_actions_detected
            != list(claim.forbidden_actions)
        ):
            raise _Blocked(reason)
    ...
    def _validate_history_graph(
        ...
        *,
        ...
        invocation_claims = [item[1] for item in history.invocation_claims]
        ...
        target_run_ids = {
            item.exact_run_id
            for item in exact_run_reservations
            if item.exact_run_reservation_id == exact_run_reservation_id
        }
        self._require_unique_invocation_claims(
            invocation_claims,
            exact_run_reservation_id=exact_run_reservation_id,
            exact_worker_start_reservation_id=(
                exact_worker_start_reservation_id
        ...
        for exact_run_reservation in exact_run_reservations:
            ...
            reason: CrossTaskExactWorkerInvocationClaimBlockedReason = (
                ...
                == exact_run_reservation_id
                else "exact_worker_invocation_claim_history_conflict"
            )
        ...
        for worker_start_reservation in worker_start_reservations:
            ...
            reason = (
                "exact_worker_invocation_claim_worker_start_reservation_invalid"
                if worker_start_reservation.exact_worker_start_reservation_id
                == exact_worker_start_reservation_id
                else "exact_worker_invocation_claim_history_conflict"
            )
        ...
        for claim in invocation_claims:
            root_matches = [
                root
                for root in roots
                if root.record_id == claim.continuation_root_record_id
            ]
            package_matches = [
                package
                for package in packages
                if package.package_id == claim.instruction_package_id
            ]
            exact_run_matches = [
                item
                for item in exact_run_reservations
            ...
            self._validate_claim_history_binding(
                claim,
                root_matches[0],
                package_matches[0],
                exact_run_matches[0],
                worker_start_matches[0],
                reason=reason,
            )
    ...
    @staticmethod
    def _require_unique_invocation_claims(
        claims: list[ProjectDirectorCrossTaskExactWorkerInvocationClaim],
        *,
        exact_run_reservation_id: UUID,
        exact_worker_start_reservation_id: UUID,
        target_run_ids: set[UUID],
    ) -> None:
        collections = (
            [item.exact_worker_invocation_claim_id for item in claims],
            [item.worker_invocation_claim_replay_key for item in claims],
            [item.worker_invocation_claim_token for item in claims],
            [item.exact_worker_start_reservation_id for item in claims],
            [item.exact_run_reservation_id for item in claims],
            [item.exact_run_id for item in claims],
        )
        if not any(
        ...
        if any(
            claim.exact_worker_start_reservation_id
            == exact_worker_start_reservation_id
            or claim.exact_run_reservation_id == exact_run_reservation_id
            or claim.exact_run_id in target_run_ids
            for claim in claims
        ):
            raise _Blocked("exact_worker_invocation_claim_replay_conflict")
        raise _Blocked("exact_worker_invocation_claim_history_conflict")
    ...
    @classmethod
    def _validate_claim_history_binding(
        cls,
        claim: ProjectDirectorCrossTaskExactWorkerInvocationClaim,
        root: ProjectDirectorCrossTaskContinuationRoot,
        package: ProjectDirectorNextTaskInstructionPackage,
        ...
        *,
        ...
        cls._validate_worker_start_history_binding(
            worker_start_reservation,
            root,
            package,
            exact_run_reservation,
        ...
        if (
            claim.continuation_sequence_no != 4
            or claim.previous_record_id
            != worker_start_reservation.exact_worker_start_reservation_id
            or claim.replay_of_record_id is not None
            or claim.continuation_id != root.continuation_id
            or claim.continuation_root_record_id != root.record_id
            or claim.continuation_root_fingerprint
            != root.continuation_fingerprint
            or claim.continuation_idempotency_key != root.idempotency_key
            or claim.instruction_package_id != package.package_id
            or claim.instruction_package_fingerprint
            != package.package_fingerprint
            or claim.instruction_candidate_fingerprint
            != package.instruction_candidate_fingerprint
            or claim.exact_run_reservation_id
            != exact_run_reservation.exact_run_reservation_id
            or claim.exact_run_reservation_fingerprint
            != exact_run_reservation.reservation_fingerprint
            or claim.exact_run_reservation_replay_key
            != exact_run_reservation.reservation_replay_key
            or claim.exact_worker_start_reservation_id
            != worker_start_reservation.exact_worker_start_reservation_id
            or claim.exact_worker_start_reservation_fingerprint
            != worker_start_reservation.worker_start_reservation_fingerprint
            or claim.exact_worker_start_reservation_replay_key
            != worker_start_reservation.worker_start_reservation_replay_key
            or claim.session_id != package.session_id
            or claim.project_id != package.project_id
            or claim.plan_version_id != package.plan_version_id
            or claim.task_creation_record_id != package.task_creation_record_id
            or claim.source_task_id != package.source_task_id
            or claim.source_run_id != package.source_run_id
            or claim.source_completion_evidence_id
            != package.source_completion_evidence_id
            or claim.source_completion_evidence_fingerprint
            != package.source_completion_evidence_fingerprint
            or claim.next_task_id != exact_run_reservation.next_task_id
            or claim.next_task_index != exact_run_reservation.next_task_index
            or claim.task_count != exact_run_reservation.task_count
            or claim.task_human_status_before
            != exact_run_reservation.task_human_status
            or claim.task_human_status_before
            != worker_start_reservation.task_human_status
            or claim.exact_run_id != exact_run_reservation.exact_run_id
            or claim.exact_run_id != worker_start_reservation.exact_run_id
            or claim.exact_run_started_at
            != exact_run_reservation.exact_run_started_at
            or claim.exact_run_started_at
            != worker_start_reservation.exact_run_started_at
            or claim.exact_run_created_at
            != exact_run_reservation.exact_run_created_at
            or claim.exact_run_created_at
            != worker_start_reservation.exact_run_created_at
            or claim.worker_model_name
            != worker_start_reservation.worker_model_name
            or claim.worker_model_tier
            != worker_start_reservation.worker_model_tier
            or claim.worker_owner_role_code
            != worker_start_reservation.worker_owner_role_code
            or claim.worker_upstream_role_code
            != worker_start_reservation.worker_upstream_role_code
            or claim.worker_downstream_role_code
            != worker_start_reservation.worker_downstream_role_code
            or claim.worker_selected_skills
            != worker_start_reservation.worker_selected_skills
            or claim.worker_repository_binding
            != worker_start_reservation.worker_repository_binding
            or claim.worker_workspace_binding
            != worker_start_reservation.worker_workspace_binding
            or claim.worker_allowed_paths
            != worker_start_reservation.worker_allowed_paths
            or claim.worker_forbidden_paths
            != worker_start_reservation.worker_forbidden_paths
            or claim.forbidden_actions
            != cls._claim_forbidden_actions(reason=reason)
        ):
            raise _Blocked(reason)
    ...
    def _locate_exact_graph(
        ...
        *,
        ...
        if len(root_matches) != 1:
            raise _Blocked("exact_worker_invocation_claim_history_invalid")
        package_matches = [
            package
        ...
        if len(package_matches) != 1:
            raise _Blocked("exact_worker_invocation_claim_package_invalid")
        exact_run_matches = [
            reservation
        ...
        worker_start_matches = [
            ...
            if reservation.exact_worker_start_reservation_id
            == exact_worker_start_reservation_id
        ]
        if len(worker_start_matches) != 1:
            raise _Blocked(
                "exact_worker_invocation_claim_worker_start_reservation_invalid"
            )
        ...
        if (
            worker_start_reservation.session_id != session_id
            or worker_start_reservation.project_id != project_id
        ):
            raise _Blocked(
                "exact_worker_invocation_claim_worker_start_reservation_invalid"
            )
        if (
            ...
            is not False
            or worker_start_reservation.product_runtime_git_write_allowed
            is not False
        ):
            raise _Blocked(
        ...
        if (
            worker_start_reservation.worker_start_reserved is not True
            or worker_start_reservation.worker_called is not False
            or worker_start_reservation.agent_session_created is not False
            or worker_start_reservation.invocation_claim_created is not False
            or worker_start_reservation.worker_outcome_recorded is not False
            or worker_start_reservation.runtime_started is not False
        ):
            raise _Blocked(
                "exact_worker_invocation_claim_worker_start_reservation_invalid"
            )
        ...
        self._validate_root_package_binding(
            root,
            package,
            reason="exact_worker_invocation_claim_package_invalid",
        )
        self._validate_exact_run_history_binding(
            exact_run_reservation,
            root,
    ...
    def _validate_package_authority(
        ...
        if (
            ...
            or any(
                ...
                for path in package.allowed_paths + package.forbidden_paths
            )
        ):
            raise _Blocked(
        ...
        cls._routing_inputs(
            package,
            reason="exact_worker_invocation_claim_worker_authority_conflict",
        )
    ...
    def _load_exact_task(self, task_id: UUID) -> Task:
        ...
        if task is None:
            raise _Blocked("exact_worker_invocation_claim_task_missing")
        return task
    ...
    def _validate_task(
        *,
        ...
        if (
            ...
            or task.id != worker_start_reservation.next_task_id
    ...
    def _load_exact_run(self, run_id: UUID) -> Run:
        ...
        if run is None:
            raise _Blocked("exact_worker_invocation_claim_run_missing")
        return run
    ...
    def _validate_run(
        ...
        *,
        ...
        if (
            ...
            or run.id != worker_start_reservation.exact_run_id
            or run.task_id != exact_run_reservation.next_task_id
            or run.task_id != worker_start_reservation.next_task_id
        ...
        if (
            ...
            or run.started_at != worker_start_reservation.exact_run_started_at
            or run.created_at != exact_run_reservation.exact_run_created_at
            or run.created_at != worker_start_reservation.exact_run_created_at
        ...
        routing_breakdown, strategy_decision = cls._routing_inputs(
            package,
            reason="exact_worker_invocation_claim_run_routing_conflict",
        )
        strategy = package.selected_strategy
        if (
            ...
            sorted(
                (
                    ...
                    in {
                        ...
                        AgentSessionStatus.REVIEW_REWORK,
                    }
                ),
                key=str,
            )
    ...
    def _build_claim(
        ...
        *,
        ...
        claim_token = (
            ProjectDirectorCrossTaskExactWorkerInvocationClaim
            .compute_worker_invocation_claim_token(
                exact_worker_invocation_claim_id=(
                    exact_worker_invocation_claim_id
                ),
                worker_invocation_claim_replay_key=replay_key,
                exact_worker_start_reservation_id=(
                    worker_start_reservation.exact_worker_start_reservation_id
                ),
                exact_worker_start_reservation_fingerprint=(
        ...
        payload: dict[str, Any] = {
            ...
            "active_run_ids_before": active_run_ids_before,
            "active_agent_session_ids_before": (
                active_agent_session_ids_before
            ),
            "worker_model_name": worker_start_reservation.worker_model_name,
            "worker_model_tier": worker_start_reservation.worker_model_tier,
            "worker_owner_role_code": (
                worker_start_reservation.worker_owner_role_code
            ),
            ...
            "worker_selected_skills": (
                worker_start_reservation.worker_selected_skills
            ),
            "worker_repository_binding": (
                worker_start_reservation.worker_repository_binding
            ),
            "worker_workspace_binding": (
                worker_start_reservation.worker_workspace_binding
            ),
            ...
            "product_runtime_git_write_allowed": False,
            "forbidden_actions": cls._claim_forbidden_actions(
                reason=reason
            ),
        }
        try:
            fingerprint = compute_p24_contract_sha256(payload)
            return ProjectDirectorCrossTaskExactWorkerInvocationClaim(
                **payload,
                worker_invocation_claim_fingerprint=fingerprint,
            )
        except (TypeError, ValueError, ValidationError) as exc:
            raise _Blocked(reason) from exc
    ...
    def _claim_forbidden_actions(
        *,
        ...
        if (
            ...
            or "worker_invocation" in default
            or not required.issubset(default)
        ):
            raise _Blocked(reason)
        return default
        ...
        *,
        ...
        try:
            return ProjectDirectorMessage(
                ...
                forbidden_actions_detected=list(claim.forbidden_actions),
                token_count=None,
                estimated_cost=None,
                created_at=claim.created_at,
            )
    ...
    def _post_write_validate(
        ...
        try:
            validated = (
                ProjectDirectorCrossTaskExactWorkerInvocationClaim
                .model_validate(claim.model_dump(mode="python"))
            )
        except (TypeError, ValueError, ValidationError) as exc:
            raise _Blocked(reason) from exc
        if (
            validated.worker_invocation_claim_fingerprint
            != validated.compute_fingerprint()
            or validated.worker_invocation_claim_replay_key
            != validated.compute_worker_invocation_claim_replay_key(
                continuation_id=validated.continuation_id,
                exact_worker_start_reservation_id=(
                    validated.exact_worker_start_reservation_id
                ),
                exact_run_reservation_id=(
                    validated.exact_run_reservation_id
                ),
            ...
            or validated.worker_invocation_claim_token
            != validated.compute_worker_invocation_claim_token(
                exact_worker_invocation_claim_id=(
                    validated.exact_worker_invocation_claim_id
                ),
                worker_invocation_claim_replay_key=(
                    validated.worker_invocation_claim_replay_key
                ),
                exact_worker_start_reservation_id=(
                    validated.exact_worker_start_reservation_id
                ),
                exact_worker_start_reservation_fingerprint=(
                    validated.exact_worker_start_reservation_fingerprint
                ),
                exact_run_id=validated.exact_run_id,
            )
        ):
            raise _Blocked(reason)
        parsed = self._parse_invocation_claim_message(
            message,
            reason=reason,
        )
        if parsed != validated:
    ...
    def _require_shared_session(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                "P24-E3B repositories must share one SQLAlchemy session"
            )
