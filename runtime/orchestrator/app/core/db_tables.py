...
class RunTable(ORMBase):
    ...
    cache_source: Mapped[str | None] = mapped_column(String(40), nullable=True)
...
class ProjectDirectorSkillBindingConfigTable(ORMBase):
    """Project-level AI Director skill binding suggestion config.

    Review-only persistence. Confirming this row does not create
    ProjectRoleSkillBinding rows, enable Skills, dispatch Workers, start Runs,
    create Agent Sessions, or call providers.
    """

    __tablename__ = "project_director_skill_binding_configs"
    __table_args__ = (
        UniqueConstraint(
            "project_id",
            name="uq_skill_binding_configs_project",
        ),
        UniqueConstraint(
            "plan_version_id",
            name="uq_skill_binding_configs_plan_version",
        ),
    )
    ...
    source_draft_id: Mapped[str] = mapped_column(String(120), nullable=False)
    status: Mapped[SkillBindingConfigStatus] = mapped_column(
        Enum(
    ...
    skill_bindings_json: Mapped[str] = mapped_column(Text, nullable=False, default="[]")
    warnings_json: Mapped[str] = mapped_column(Text, nullable=False, default="[]")
    review_note: Mapped[str] = mapped_column(Text, nullable=False, default="")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
...
class ProjectDirectorRepositoryBindingConfigTable(ORMBase):
    """Project-level AI Director repository binding suggestion config.

    Review-only persistence. Confirming this row does not create
    RepositoryWorkspace rows, write repositories, invoke git/apply actions,
    dispatch Workers, start Runs, create Agent Sessions, or call providers.
    """

    __tablename__ = "project_director_repository_binding_configs"
    __table_args__ = (
        UniqueConstraint(
            "project_id",
            name="uq_repository_binding_configs_project",
        ),
        UniqueConstraint(
            "plan_version_id",
            name="uq_repository_binding_configs_plan_version",
        ),
    )
    ...
    source_draft_id: Mapped[str] = mapped_column(String(120), nullable=False)
    status: Mapped[RepositoryBindingConfigStatus] = mapped_column(
        Enum(
            RepositoryBindingConfigStatus,
            native_enum=False,
            values_callable=_enum_values,
    ...
    repository_bindings_json: Mapped[str] = mapped_column(
        Text, nullable=False, default="[]"
    )
    warnings_json: Mapped[str] = mapped_column(Text, nullable=False, default="[]")
    review_note: Mapped[str] = mapped_column(Text, nullable=False, default="")
...
class ProjectDirectorVerificationConfigTable(ORMBase):
    """Project-level AI Director verification mechanism suggestion config.

    Review-only persistence. Confirming this row does not execute commands,
    create Run rows, dispatch Workers, call providers, invoke subprocess/os.system,
    invoke planning/apply/apply-local/git-commit, or write repositories.
    """

    __tablename__ = "project_director_verification_configs"
    __table_args__ = (
        UniqueConstraint(
            "project_id",
            name="uq_verification_configs_project",
        ),
        UniqueConstraint(
            "plan_version_id",
            name="uq_verification_configs_plan_version",
        ),
    )
    ...
    source_draft_id: Mapped[str] = mapped_column(String(120), nullable=False)
    status: Mapped[VerificationConfigStatus] = mapped_column(
        Enum(
