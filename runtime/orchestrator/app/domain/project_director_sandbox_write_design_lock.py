"""Design-lock result domain for Project Director P21-B sandbox writes."""
...
SandboxWriteDesignLockMode = Literal["dry_run", "fake_lock"]
SandboxWriteDesignLockStatus = Literal["locked", "blocked"]


class ProjectDirectorSandboxWriteDesignLockResult(DomainModel):
    """P21-B design lock that records future write requirements only."""

    design_lock_status: SandboxWriteDesignLockStatus
    session_id: UUID
    ...
    source_execution_mode: str | None = Field(default=None, max_length=120)
    ...
    design_lock_summary: str = Field(min_length=1, max_length=2_000)
    ...
    @classmethod
    def reject_runtime_write_and_start_flags(cls, value: bool) -> bool:
        if value:
            raise ValueError(
                "P21-B design lock runtime side-effect flags must remain false"
            )
        return value
