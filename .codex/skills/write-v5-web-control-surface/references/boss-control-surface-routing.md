...
3. 什么时候当前线程应先让位给 `govern-v5-web-structure`
...
2. 若结构明确且现有目录装不下，再新增 `apps/web/src/features/agent-teams/`
...
1. 先增强 `ProjectOverviewPage` 等现有老板入口的最小挂载点
...
## 4. 让位给 `govern-v5-web-structure` 的前置信号

出现下面任一情况，先不要继续路由控制面实现：

- 为了挂入口，必须先改 `App.tsx` / `ProjectOverviewPage.tsx` 的大块结构
- 当前 feature 目录装不下，但新 feature 的 owner 边界还说不清
- 当前页面已经同时承担多个 feature 的具体实现
- 这次主要要做的是 section 拆分、hooks 下沉、测试锚点补齐

这时先让 `govern-v5-web-structure` 决定：

- 控制面应该挂在哪个 feature / section
- 顶层聚合页还能承受多少入口挂载
- 是否需要先抽文件、补 testid、收口 owner

## 5. 线程结束前至少回答

- 这次控制面挂在哪个 feature
- 为什么不放到别的 feature
- 是否触发了 `govern-v5-web-structure`
- 依赖哪些后端字段 / 接口
- 当前是否已可交互
- 下一线程是接 `structure`、`backend`、`verify`、`review` 还是 `docs`
